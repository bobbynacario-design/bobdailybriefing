...
test('a saved meeting brief keeps its line breaks, like a dossier',()=>{
  const state=core.createSet({},'Briefs',now,'briefs').state;
  const saved=core.addItem(state,'briefs',{id:'meeting:m1:brief',source:'Meeting',title:'Meeting brief: Suncorp',detail:'Meeting brief · Suncorp\nReserves lifted.\nQ1. Ask?'},now).state;
  assert.equal(saved.sets[0].items[0].detail,'Meeting brief · Suncorp\nReserves lifted.\nQ1. Ask?');
});
