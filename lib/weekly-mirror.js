...
(function(root) {
  ...
  var state = {uid:null, mirrors:{}, loadedAt:0, loading:false, running:false, status:'', shown:'', openedFor:'', answering:'', answerDraft:'', answerSaving:false, answerMessage:'', answerShownFor:''};
  ...
  // ── Write my answer ──
  // The question used to end the read. Bob can answer it here; the answer is
  // saved on the read with the question it answers, and the next read follows
  // up on what he wrote (functions/weekly-mirror.js).
  function answerBlock(mirror) {
    var key = mirror.weekKey, answer = mirror.answer && String(mirror.answer.text || '').trim() ? mirror.answer : null;
    var box = node('div', 'boost-mirror-answer');
    if (state.answering === key) {
      var label = node('label', '', 'Your answer'), input = node('textarea'), actions = node('div', 'boost-mirror-answer-actions');
      var save = node('button', 'tool-chip', 'Save answer'), cancel = node('button', 'tool-chip', 'Cancel');
      input.maxLength = 800; input.rows = 3; input.value = state.answerDraft; input.placeholder = 'A sentence or two is plenty.';
      input.addEventListener('input', function() { state.answerDraft = input.value; });
      save.type = 'button'; cancel.type = 'button'; save.disabled = state.running || state.answerSaving;
      save.addEventListener('click', function() { saveAnswer(mirror); });
      cancel.addEventListener('click', function() { state.answering = ''; state.answerMessage = ''; render(); });
      label.appendChild(input); actions.append(save, cancel);
      box.append(label, actions, node('p', 'boost-kicker', state.answerMessage || (state.running ? 'Saving waits until the read finishes.' : '')));
      return box;
    }
    if (answer) {
      box.appendChild(node('h4', '', 'Your answer'));
      if (answer.question && answer.question !== mirror.question) box.appendChild(node('p', 'boost-mirror-why', 'To an earlier wording: “' + answer.question + '”'));
      box.appendChild(node('p', 'boost-mirror-answer-text', answer.text));
    }
    var open = node('button', 'tool-chip', answer ? 'Edit my answer' : 'Write my answer');
    open.type = 'button';
    open.addEventListener('click', function() { state.answering = key; state.answerDraft = answer ? answer.text : ''; state.answerMessage = ''; render(); });
    box.appendChild(open);
    if (state.answerMessage && state.answerShownFor === key) box.appendChild(node('p', 'boost-kicker', state.answerMessage));
    return box;
  }
  function saveAnswer(mirror) {
    var uid = state.uid, session = state, key = mirror.weekKey, text = String(state.answerDraft || '').trim().slice(0, 800);
    if (!uid || state.running || state.answerSaving || typeof root.fbSaveWeeklyMirrorAnswer !== 'function') return;
    var answer = text ? {text:text, question:mirror.question, at:new Date().toISOString()} : null;
    state.answerSaving = true; render();
    root.fbSaveWeeklyMirrorAnswer(uid, key, answer).then(function() {
      if (session !== state || uid !== root._firebaseUid) return;
      state.answerSaving = false; state.answering = ''; state.answerShownFor = key;
      if (state.mirrors[key]) state.mirrors[key].answer = answer;
      state.answerMessage = answer ? 'Saved. Your next read will follow up on it.' : 'Answer cleared.';
      render();
    }, function() {
      if (session !== state || uid !== root._firebaseUid) return;
      state.answerSaving = false; state.answerMessage = 'Could not save your answer. It is still in the box; try again.';
      render();
    });
  }
  function body(mirror) {
    ...
    // Your goals (lib/daily-boost.js), each read against the week.
    if ((mirror.goals || []).length) {
      var goalPart = node('section', 'boost-mirror-part'), goalList = node('ul');
      mirror.goals.forEach(function(item) {
        var li = node('li');
        li.append(node('strong', '', item.goal), document.createTextNode(item.read ? ' — ' : ''));
        if (item.read) appendCitations(li, item.read);
        goalList.appendChild(li);
      });
      goalPart.append(node('h4', '', 'Your goals this week'), goalList);
      parts.push(goalPart);
    }
    ...
    if (tryNext.action) {
      ...
      var controls = node('div','boost-library-controls'), label = node('label','','Revisit on'), due = node('input'), choose = node('button','tool-chip','Try this'), decide = node('button','tool-chip','Open as decision'), feedback = node('p','boost-kicker');
      due.type = 'date'; due.min = todayKey(); due.value = new Date(Date.parse(todayKey() + 'T00:00:00Z') + 7*86400000).toISOString().slice(0,10);
      choose.setAttribute('data-track', 'mirror_try'); decide.setAttribute('data-track', 'mirror_decision');
      label.appendChild(due); controls.append(label,choose,decide); box.append(controls,feedback);
      ...
      decide.addEventListener('click',function() {
        if (typeof root.openDecisionFromInsight !== 'function') { feedback.textContent = 'Open the Decision Journal to capture this.'; return; }
        root.openDecisionFromInsight({title:tryNext.action, source:'weekly', detail:['Weekly read-back · ' + mirror.weekKey,tryNext.why || '',mirror.question ? 'Question to sit with: ' + mirror.question : ''].filter(Boolean).join('\n'),
          reviewDate:due.value, label:'Weekly read-back · ' + mirror.weekKey});
      });
      parts.push(box);
    }
    parts.push(node('blockquote', 'boost-mirror-question', mirror.question));
    parts.push(answerBlock(mirror));
  ...
  function render() {
    ...
    var waiting = state.saving ? 'Saving your latest notes…' : state.running ? 'Reading your week. Usually about half a minute; your notes go to OpenAI for this read only.' : '';
    // The app's live ring and timer while it works (aiWorkingHtml escapes the text).
    if (waiting && typeof root.aiWorkingHtml === 'function') el('status').innerHTML = root.aiWorkingHtml(waiting, {since:state.runStarted, slow:state.running && !state.saving ? 90 : 0});
    else el('status').textContent = waiting || state.status;
    var select = el('week');
  ...
  async function run() {
    ...
    state.running = true; state.runStarted = Date.now(); state.status = ''; state.saving = typeof root.flushDailyBoost === 'function'; render();
    try {
  ...
  root.renderWeeklyMirror = function() {
    ...
    if (uid !== state.uid) state = {uid:uid, mirrors:{}, loadedAt:0, loading:false, running:false, status:'', shown:'', openedFor:state.openedFor, answering:'', answerDraft:'', answerSaving:false, answerMessage:'', answerShownFor:''};
    render(); load();
  };
