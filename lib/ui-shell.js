...
(function(root) {
  ...
  root.commandOpenItem = async function(encodedId) {
    ...
    var title = item.source === 'Radar' ? item.id.replace(/^radar-/,'') : item.source === 'Decisions' ? item.title.replace(/^(Review open call: |Review due: |Today: )/,'') :
      item.source === 'Experiments' ? item.title.replace(/^Experiment to review: /,'') : item.source === 'Accounts' ? (item.target || item.title) : item.title;
    if (!root.highlightSourceTitle(title,item.page,item.id)) root.showToast('Source opened. This item is not in the current view; check its filters or archive.','info');
      ...
      // Today redraws some panels as it opens (the experiments list among them);
      // if that replaced the highlighted element, find it again.
      var view = document.getElementById('page-' + item.page);
      if (view && !view.querySelector('.source-highlight')) root.highlightSourceTitle(title,item.page,item.id);
    }, 400);
