...
test('an experiment comes back due, retains its outcome, and can be reopened',()=>{
  ...
  const outcome=row.children[2].children[0]; outcome.value='The reader found the main point faster.'; outcome.handlers.input(); row.children[3].handlers.click();
  ...
  row.children[3].handlers.click(); assert.match(element('experiments').children[0].children[1].textContent,/Ready to revisit/);
});
...
test('reminders are set from a card, come due on Today, and can be checked, noted, snoozed or cancelled',()=>{
  ...
  assert.equal(element('note').value,'Checked “APRA quarterly claims data, due 2 Oct” (on “Insurer lifts BI reserves”): ','noting what you found starts the note');
  assert.equal(context.dailyBoostReminderFor(apra),null,'and marks it checked');
...
test('an aha check joins To check, not the experiment slot, and is not added twice',()=>{
  ...
  assert.match(context.addDailyBoostCheck({metric:'eleventh',headline:'x',due:'2026-10-02'}).message,/ten checks/);
});
