...
test('on Sundays the collapsed strip offers the weekly read until it has been read',()=>{
  let read=false;
  const {context,element}=setup({now:'2026-09-27T04:00:00Z',weeklyMirrorReadOn:()=>read});
  element('done').handlers.click(); context.renderDailyBoost();
  assert.match(element('panel').className,/is-compact/,'done: the panel collapses to its strip');
  assert.equal(element('compact-week').hidden,false);
  element('compact-week').handlers.click();
  assert.equal(element('mirror-panel').open,true,'opens the read');
  assert.doesNotMatch(element('panel').className,/is-compact/,'inside the expanded panel');
  read=true; context.renderDailyBoost();
  assert.equal(element('compact-week').hidden,true,'gone once read');
  const {element:weekday}=setup();
  assert.equal(weekday('compact-week').hidden,true,'not on other days');
});
// ── Your goals ──
test('a goal theme favours its sparks gently, like More like this, never taking over',()=>{
  const {context}=setup(), core=context.DailyBoostCore;
  const run=lean=>{ const saved={}, seen=[]; for (let i=0;i<28;i++) { const d=new Date(Date.UTC(2026,8,21+i)).toISOString().slice(0,10); const spark=core.sparkFor(d,core.clean(saved),lean); saved[d]={spark,updatedAt:1}; seen.push(core.themes[spark]); } return seen; };
  const plain=run(), leaned=run(['Craft']), count=(list,theme)=>list.filter(t=>t===theme).length;
  assert.ok(count(leaned,'Craft')>count(plain,'Craft'),'it leans towards the goal');
  assert.ok(count(leaned,'Craft')<=8,'about one day in four at most');
  assert.ok(new Set(leaned).size>=8,'the other themes still come round');
  leaned.forEach((t,i)=>{ if (i) assert.notEqual(t,leaned[i-1],'no theme two days running'); });
  assert.deepEqual([...run(undefined)],[...plain],'no goals, no change');
});
test('goals are set in the editor, ticked by day, counted over two weeks, and synced',()=>{
  const saves=[], day='2026-09-25';
  const {context,element,saved}=setup({fbSaveGoals:async(uid,goals)=>{saves.push({uid,goals:JSON.parse(JSON.stringify(goals))});}});
  assert.equal(element('goal-ticks').hidden,true,'no goals, no ticks');
  element('goals-edit').handlers.click();
  const rows=()=>element('goals-editor').children.filter(c=>c.className==='boost-goal-edit');
  const save=()=>element('goals-editor').children.find(c=>c.className==='boost-goals-actions').children[0].handlers.click();
  assert.equal(rows().length,3);
  rows()[0].children[0].value='Be consistent with what I want'; rows()[0].children[0].handlers.input();
  rows()[0].children[1].value='Draft the first page'; rows()[0].children[2].value=day;
  assert.equal(rows()[0].children[3].value,'Momentum','a theme guessed from the words');
  rows()[1].children[0].value='Build a BI advisory offer'; rows()[1].children[0].handlers.input();
  assert.equal(rows()[1].children[3].value,'Craft');
  save();
  const stored=JSON.parse(saved.get('bob-goals:alice'));
  assert.deepEqual(stored.map(g=>g.text),['Be consistent with what I want','Build a BI advisory offer']);
  assert.equal(saves.length,1); assert.equal(saves[0].uid,'alice');
  assert.equal(element('goals-summary').textContent,'Your goals · 2');
  assert.equal(stored[0].nextMove,'Draft the first page'); assert.equal(stored[0].nextDue,day);
  assert.match(element('start-main').textContent,/Draft the first page/,'a due next move reaches Start here');
  assert.equal(element('goal-ticks').hidden,false);
  assert.equal(element('goal-tick-list').children.length,2);
  element('goal-tick-list').children[0].handlers.click();
  assert.deepEqual([...JSON.parse(saved.get('bob-daily-boost:alice'))[day].goalTicks],[stored[0].id]);
  assert.match(element('goals-list').children[0].children[1].textContent,/moved on 1 day of the last 14/);
  element('goals-list').children[0].children[3].handlers.click();
  const moved=JSON.parse(saved.get('bob-goals:alice'))[0];
  assert.equal(moved.nextMove,'','finishing clears the move so the next one can be set');
  assert.equal(element('goals-editor').hidden,false,'the editor opens for the next move');
  assert.match(element('goals-status').textContent,/Move captured/);
  element('goals-editor').children.find(c=>c.className==='boost-goals-actions').children[1].handlers.click();
  element('goals-edit').handlers.click();
  rows()[1].children[0].value=''; save();
  const after=JSON.parse(saved.get('bob-goals:alice'));
  assert.equal(after.filter(g=>!g.archived).length,1);
  assert.equal(after.filter(g=>g.archived).length,1,'a cleared goal is archived, so its ticks keep their meaning');
});
test('the why line names the goal when a goal moved the pick',()=>{
  const {context:probe}=setup(), core=probe.DailyBoostCore, day='2026-09-25';
  const base=core.sparkFor(day,{}), target=core.order.find(id=>core.themes[id]!==core.themes[base]);
  const goals=JSON.stringify([{id:'gabc1',text:'A goal of mine',theme:core.themes[target],archived:false}]);
  const {element}=setup({localStorage:{getItem:k=>k==='bob-goals:alice'?goals:null,setItem:()=>{}}});
  assert.ok(element('why').textContent.includes('For your goal “A goal of mine”'),element('why').textContent);
});
test('the Morning 5 gets the oldest open experiment whose revisit date has come',()=>{
  const {context,saved}=setup(), today=context.DailyBoostCore.dateKey();
  const back=n=>new Date(Date.parse(today+'T00:00:00Z')-n*86400000).toISOString().slice(0,10);
  saved.set('bob-daily-boost:dave',JSON.stringify({
    [back(6)]:{spark:1,trialPlan:'Send the shorter request',trialDue:back(1),updatedAt:1},
    [back(8)]:{spark:2,trialPlan:'Ask one clearer question',trialDue:back(2),updatedAt:1},
    [back(9)]:{spark:3,trialPlan:'Already reviewed',trialDue:back(5),trialDone:true,updatedAt:1},
    [back(3)]:{spark:4,trialPlan:'Not yet',trialDue:back(-3),updatedAt:1}
  }));
  context._firebaseUid='dave'; context.renderDailyBoost();
  assert.deepEqual({...context.dailyBoostDueExperiment()},{plan:'Ask one clearer question',due:back(2),day:back(8)});
  context.resetDailyBoost();
  assert.equal(context.dailyBoostDueExperiment(),null,'signed out, nothing');
});
