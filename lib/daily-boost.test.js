...
function linkedDay() {
  const {context}=setup(), core=context.DailyBoostCore;
  for (let i=0;i<120;i++) { const day=shift('2026-09-25',i); if (core.sparkFor(day,{})===core.linked) return day; }
...
test('the briefing-linked spark names the top story, jumps to it, and keeps it once started',()=>{
  ...
  assert.equal(element('quest').textContent,'Today’s top briefing story: “Insurer lifts BI reserves after flood”. Write: “What would I need to know before acting on this?”');
  ...
  assert.equal(element('quest').textContent,'Open the original source for “Insurer lifts BI reserves after flood” (Insurance News). Separate what it establishes from what you are inferring.');
...
test('without a briefing the quest stays generic, an older briefing is labelled, and a refresh keeps the card open',()=>{
  ...
  assert.equal(element('quest').textContent,'Top story in the briefing below (Thursday, 24 September 2026): “Reinsurers reprice cat layers”. Write: “What would I need to know before acting on this?”');
