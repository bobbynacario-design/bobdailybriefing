...
function setup(extra = {}) {
  const elements = new Map(), saved = new Map(), timers = new Map();
  let timerId = 0;
  ...
  return {context,saved,events,runTimers,element:name=>element('boost-'+name)};
}
...
test('signing in merges the account copy, uploads device-only days, then writes only edited days',async()=>{
  const today=new Date().toLocaleDateString('en-CA',{timeZone:'Asia/Manila'}), yesterday=shift(today,-1);
