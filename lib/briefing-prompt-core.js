(function (root) {
  ...
  function groundingRules(evidence) {
    ...
    return [
      ...
      '- A list of real, already-fetched Australian stories appears at the end of this prompt: insurance trade news, plus electricity-network and trucking trade news for the costs his claims turn on.',
      '- Build the insurance section ONLY from that list. Do not web-search for it and do not add stories from your own knowledge.',
      '- Choose the 4 stories most relevant to Bob. If fewer than 4 are genuinely relevant, return fewer. Never pad. A network or trucking story belongs in insurance only when it moves a cost or a claim in his files; otherwise use it in interruptions or leave it.',
  ...
  function ahaRules() {
    return [
      ...
      // The 26 Sep regenerate joined a Philippine storm forecast to miss land with
      // an Australian strata mould ruling because both said "storm": a shared word
      // dressed as an insight, when null was the honest answer.
      '- A connection needs a mechanism: the same actor, market, cost, place or cause running from one story into the other. Sharing a word or a theme ("storms", "AI", "rates") is not a connection.',
      '- Every linked story must be necessary: if the read still stands with one of them removed, it is not a connection. An event that causes no loss (a storm forecast to miss land) cannot carry a claims insight.',
      '- It must bear on something that can happen in his work: a claim, a cost inside a claim, a client or a cover. If no two stories genuinely act on each other today, set aha to null; that is the usual honest answer on a thin day.',
      ...
      '- It must not restate any story\'s relevance line, and it must not be a first-order consequence ("higher rates hurt borrowers", "storms raise claims"). Among reads that hold, prefer the one that would be early rather than safe; never trade coherence for surprise.',
  ...
  function workRules() {
    return [
      ...
      '- Before filling the interruptions and global sections with general news, search specifically for: electricity network charges, AER or AEMC decisions and linesworker or contractor agreements; road-authority repair and incident costs; heavy-vehicle repair times, parts shortages, prime-mover hire and freight rates; outages and severe weather stopping Australian small businesses; and news naming his accounts. Use what you find when it clears the bar. A fetched list, where given, also carries electricity-network and trucking trade news.',
  ...
  function accountsRules(context) {
    ...
    return [
      ...
      '- Search for news on them. A story that names one and is otherwise worth his attention should be preferred, and its relevance line says what it changes for his files.',
      // The same regenerate wrote "QBE and Allianz files" into an APRA story that
      // named neither, to satisfy the line above.
      '- Name an account in a relevance line only when the story itself is about that organisation. Never add an account\'s name to a story that does not mention it; the app marks stories that name an account by itself.',
  ...
  function buildBriefingPrompt(options) {
    ...
    return [
      ...
      '    It must be something that will resolve or change on a known day: a named decision, hearing, release or data point. A database or page that simply updates every day or week ("the AFCA determinations database", "the court judgments list") is not a watch_metric.',
      ...
      '  In the markets section, never run a story whose headline is an index or currency move ("Wall Street gains…", "ASX slips…", "PSEi rebounds…").',
