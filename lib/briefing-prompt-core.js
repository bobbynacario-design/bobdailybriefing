(function (root) {
  ...
  function buildRecentBriefings(archive, todayKey) {
    ...
    var days = Object.keys(byDay).sort().reverse().slice(0, RECENT_SHOWN).map(function(key) {
      ...
      return {dateKey: key, dateLabel: shortText(briefing.date, 60), items: items, watch: shortText(briefing.watch, 220)};
    }).filter(function(day) { return day.items.length || day.watch; });
  ...
  function buildBriefingPrompt(options) {
    ...
    return [
      ...
      '  },',
      '  "sections": {',
      '    "global": [{"headline": "", "body": "", "source": "", "url": "", "relevance": "", "relevance_level": "high"}],',
  ...
  var api = {buildBriefingPrompt: buildBriefingPrompt, defaultDateLabel: defaultDateLabel, buildReaderFeedback: buildReaderFeedback, cleanAha: cleanAha, buildRecentBriefings: buildRecentBriefings, countReruns: countReruns};
