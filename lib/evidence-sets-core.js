(function (root) {
  ...
  // A saved aha or dossier is a document of lists and labelled parts; it keeps
  // its line breaks (spaces inside each line are still tidied), where flattened
  // it reads as one wall of text. Everything else stays a single line.
  var LONG_FORM = /:(aha|dossier|brief)$/;
  function lines(value, limit) {
    var result = String(value == null ? '' : value).replace(/\r\n?/g, '\n').split('\n')
  ...
  function normalizeItem(raw) {
    ...
    return {
      ...
      title: title,
      detail: LONG_FORM.test(text(raw.id)) ? lines(raw.detail, 4000) : text(raw.detail, 420),
      meta: text(raw.meta, 140),
      page: text(raw.page, 40),
      ref: text(raw.ref, 220),
      // The story's own page, when it has one (web links only).
      url: /^https?:\/\//i.test(text(raw.url, 600)) ? text(raw.url, 600) : '',
  ...
  function normalize(raw) {
    ...
    arr(raw.sets).forEach(function (candidate) {
      ...
      if (candidate.question) set.question = normalizeQuestion(candidate.question);
      sets.push(set);
    });
  ...
  function normalizeQuestion(raw) {
    raw = raw || {};
    function lines(value, max) { return String(value || '').trim().slice(0,max); }
    return {prompt:lines(raw.prompt,500),purpose:lines(raw.purpose,1000),keywords:text(raw.keywords,150),
      assessment:lines(raw.assessment,4000),gaps:lines(raw.gaps,2000),
      deadline:/^\d{4}-\d{2}-\d{2}$/.test(raw.deadline || '') ? raw.deadline : '',
      status:raw.status === 'closed' ? 'closed' : 'active',reviewedAt:raw.reviewedAt ? iso(raw.reviewedAt) : ''};
  }
  function updateQuestion(raw,id,question,now) {
    var state = copy(raw), set = findSet(state,id);
    if (!set) return result(state,false,id,'Evidence set not found');
    set.question = normalizeQuestion(question);
    if (!set.question.prompt) return result(copy(raw),false,id,'Enter a working question first');
    set.updatedAt = iso(now || Date.now());
    return result(state,true,id,'');
  }
  function researchNote(raw,id) {
    var set = findSet(normalize(raw),id);
    if (!set) return '';
    var q = set.question || {}, output = ['# '+set.name,'', '## Working question',q.prompt || set.name,'',
      '## Purpose',q.purpose || 'Not recorded.','', 'Status: '+(q.status || 'active')+(q.deadline ? ' · Due: '+q.deadline : ''),'',
      '## My assessment',q.assessment || 'No assessment recorded.','', '## Open questions and evidence to request',q.gaps || 'Not recorded.','',
      '## Saved evidence snapshots','These are saved app excerpts, not independently verified quotations. Open the original sources before relying on them.',''];
    set.items.forEach(function(item,i) {
      output.push('### ['+(i+1)+'] '+item.title,item.detail,'Source: '+item.source+(item.meta ? ' · '+item.meta : ''));
      if (item.url) output.push('Link: '+item.url);
      output.push('Captured: '+item.capturedAt,'App reference: '+(item.page || '')+' / '+(item.ref || item.id || item.key),
        'My note: '+(item.note || 'None.'),'');
    });
    if (!set.items.length) output.push('No evidence saved yet.');
    return output.join('\n');
  }
  var api = {
