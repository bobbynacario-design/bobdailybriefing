...
(function(root) {
  ...
  // "Turn a headline into a question": its quests name a real story from the briefing.
  var LINKED = 11;
  ...
  function clean(value) {
    ...
    Object.keys(value).filter(function(day) { return /^\d{4}-\d{2}-\d{2}$/.test(day); }).sort().slice(-90).forEach(function(day) {
      ...
      result[day] = {energy:item.energy === 'stretch' ? 'stretch' : 'gentle', offset:Number.isInteger(item.offset) && item.offset >= 0 ? item.offset % prompts.length : 0, done:item.done === true, tucked:item.tucked === true, note:typeof item.note === 'string' ? item.note.slice(0,1200) : '', carry:typeof item.carry === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(item.carry) ? item.carry : '', ref:typeof item.ref === 'string' ? item.ref.slice(0,300) : '', refSource:typeof item.refSource === 'string' ? item.refSource.slice(0,120) : '', updatedAt:Number.isFinite(item.updatedAt) && item.updatedAt > 0 ? Math.floor(item.updatedAt) : 0};
  ...
  root.DailyBoostCore = {dateKey:dateKey,promptFor:promptFor,sparkFor:sparkFor,nextSpark:nextSpark,clean:clean,merge:merge,review:REVIEW,linked:LINKED,count:prompts.length,themes:themes.slice(),order:order.slice()};
  ...
  function liveStory() {
    var live = typeof root.dailyBoostBriefingItem === 'function' ? root.dailyBoostBriefingItem() : null;
  ...
  function questText(item, prompt) {
    var story = item.spark === LINKED ? linkedStory(item) : null;
    if (!story) return prompt[item.energy === 'stretch' ? 3 : 2];
    var lead = story.stored ? 'The briefing story you started with' : story.isToday ? 'Today’s top briefing story' : 'Top story in the briefing below' + (story.dateLabel ? ' (' + story.dateLabel + ')' : '');
    return item.energy === 'stretch' ? 'Open the original source for “' + story.headline + '”' + (story.source ? ' (' + story.source + ')' : '') + '. Separate what it establishes from what you are inferring.' : lead + ': “' + story.headline + '”. Write: “What would I need to know before acting on this?”';
  }
  function persist() {
    if (!account) { status(); return; }
    if (current().spark === LINKED && !current().ref && liveStory()) { current().ref = liveStory().headline.slice(0,300); current().refSource = liveStory().source.slice(0,120); }
    else if (current().spark !== LINKED && current().ref) { current().ref = ''; current().refSource = ''; }
  ...
  function render() {
    ...
    renderLookback(item); renderCarried(item);
    ...
    el('briefing-jump').hidden = !(item.spark === LINKED && linkedStory(item) && !linkedStory(item).stored);
