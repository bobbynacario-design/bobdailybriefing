...
(function(root) {
  ...
  function clean(value) {
    ...
    days.forEach(function(day) {
      ...
      entry.noteVersions = (Array.isArray(item.noteVersions) ? item.noteVersions : []).filter(function(v) { return v && typeof v.text === 'string' && v.text.trim() && Number.isFinite(v.at); }).slice(-8).map(function(v) { return {text:v.text.slice(0,1200),at:v.at}; });
    });
    return result;
  }
  ...
  function mergeDay(a,b) {
    ...
    out.noteVersions = versions.filter(function(v,i,list) { return v.text !== out.note && list.findIndex(function(other) { return other.text === v.text; }) === i; }).sort(function(x,y) { return x.at-y.at || x.text.localeCompare(y.text); }).slice(-8);
    ...
    Object.keys(records).forEach(function(key) {
      ...
      FIELDS.forEach(function(field) {
        if (JSON.stringify(entry[field]) !== JSON.stringify(old && old[field])) {
          if (field === 'note') entry.noteParent = old ? fieldTime(old,'note') : 0;
  ...
  function push() {
    ...
    root.fbSaveDailyBoost(uid, entries, stale).then(function(confirmed) {
      if (uid !== account || session !== sync) return;
      if (confirmed) { records = merge(records,confirmed).records; baseline = JSON.parse(JSON.stringify(records)); saveLocal(); render(); }
      sync.inflight--;
  ...
  function render() {
    ...
    if (switched) {
      account = uid; records = {}; baseline = {}; failed = false; shownDay = null; resetSync();
  ...
  function renderPersonal() {
    ...
    Object.keys(records).forEach(function(key) { (records[key].noteVersions || []).forEach(function(version) {
      var card = document.createElement('article'), heading = document.createElement('strong'), note = document.createElement('p');
      heading.textContent = 'Alternate reflection · ' + label(key); note.textContent = version.text;
      card.append(heading,note); conflicts.appendChild(card);
    }); });
