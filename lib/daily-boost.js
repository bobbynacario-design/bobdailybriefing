...
(function(root) {
  ...
  function clean(value) {
    ...
    Object.keys(value).filter(function(day) { return /^\d{4}-\d{2}-\d{2}$/.test(day); }).sort().slice(-90).forEach(function(day) {
      ...
      result[day] = {energy:item.energy === 'stretch' ? 'stretch' : 'gentle', offset:Number.isInteger(item.offset) && item.offset >= 0 ? item.offset % prompts.length : 0, done:item.done === true, tucked:item.tucked === true, note:typeof item.note === 'string' ? item.note.slice(0,1200) : ''};
  ...
  function render() {
    ...
    el('date').textContent = label(day) + ' · ' + (themes[item.spark] === 'Reset' ? 'Rest & reset' : themes[item.spark]);
  ...
  // Reflections live only on this device, so offer a plain-text copy of all of them.
  function exportText() {
    var days = Object.keys(records).filter(function(key) { return won(records[key]); }).sort().reverse();
    var lines = ['Daily Boost reflections · copied ' + day + ' · from this device', ''];
    days.forEach(function(key) {
      var entry = records[key];
      lines.push(label(key) + ' ' + key.slice(0,4) + ' · ' + prompts[entry.spark][0] + ' (' + themes[entry.spark] + ')' + (entry.done ? ' · done' : ''));
  ...
  root.renderDailyBoost = function() { expanded = false; render(); };
  root.resetDailyBoost = function() { records = {}; account = null; day = null; expanded = false; if (el('note')) { el('note').value = ''; el('history').replaceChildren(); el('feedback').textContent = ''; el('export').value = ''; el('export').hidden = true; el('copy-status').textContent = ''; } };
