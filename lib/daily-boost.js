...
(function(root) {
  ...
  function clean(value) {
    ...
    Object.keys(value).filter(function(day) { return /^\d{4}-\d{2}-\d{2}$/.test(day); }).sort().slice(-90).forEach(function(day) {
      ...
      result[day] = {energy:item.energy === 'stretch' ? 'stretch' : 'gentle', offset:Number.isInteger(item.offset) && item.offset >= 0 ? item.offset % prompts.length : 0, done:item.done === true, tucked:item.tucked === true, note:typeof item.note === 'string' ? item.note.slice(0,1200) : '', carry:typeof item.carry === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(item.carry) ? item.carry : '', ref:typeof item.ref === 'string' ? item.ref.slice(0,300) : '', refSource:typeof item.refSource === 'string' ? item.refSource.slice(0,120) : '', stories:cleanStories(item.stories), updatedAt:Number.isFinite(item.updatedAt) && item.updatedAt > 0 ? Math.floor(item.updatedAt) : 0};
  ...
  function current() { return records[day] || (records[day] = {energy:'gentle',spark:sparkFor(day,records),offset:0,done:false,note:'',stories:[]}); }
  function status() {
    var text;
  ...
  function persist() {
    ...
    if (live && !current().ref) { current().ref = live.headline.slice(0,300); current().refSource = live.source.slice(0,120); }
    else if (!BRIEFING[current().spark] && current().ref) { current().ref = ''; current().refSource = ''; }
  ...
  function renderLibrary() {
    ...
    prompts.forEach(function(prompt,index) {
      ...
      button.addEventListener('click',function() {
        ...
        current().spark = index; current().ref = ''; current().refSource = ''; persist(); render();
  ...
  document.addEventListener('DOMContentLoaded',function() {
    ...
    el('swap').addEventListener('click',function() {
      render();
      if (!current().done) { current().spark = nextSpark(current().spark,records,day); current().ref = ''; current().refSource = ''; }
      persist(); render();
    });
