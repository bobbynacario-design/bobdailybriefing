...
(function(root) {
  ...
  function pickFrom(position, saved, today, excluded) {
    ...
    choices.forEach(function(id) { var score = preferenceScore(id, saved, today); if (score > top && !lately[themes[id]]) { favoured = id; top = score; } });
  ...
  function nudgedToday(spark, saved, today) {
    var choices = choicesFrom(indexFor(today,0,order.length), saved, today);
    return !!choices.length && spark !== rotationPick(choices, saved, today) && preferenceScore(spark, saved, today) >= NUDGE_MARGIN;
  }
  function preferenceScore(id, saved, today) {
  ...
  function nextSpark(from, saved, today) {
    ...
    return pickFrom(position < 0 ? indexFor(today,0,order.length) : position + 1, saved, today, from);
  }
  ...
  var FIELDS = ['energy','spark','done','tucked','note','carry','ref','refSource','refUrl','picked','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource','trialResult','trialNext','sparkResponse','resurfaced'];
  ...
  var ARCHIVE_FIELDS = ['energy','spark','done','note','carry','ref','refSource','refUrl','stories','reminders','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource','trialResult','trialNext','sparkResponse','resurfaced','updatedAt'];
  ...
  function worthKeeping(entry) {
    return !!entry && (won(entry) || entry.stories.length || entry.reminders.length || !!entry.trialPlan || !!entry.ref || !!entry.sparkResponse || !!(entry.resurfaced && entry.resurfaced.action));
  }
  ...
  function current() { return records[day] || (records[day] = {energy:'gentle',spark:sparkFor(day,records),offset:0,done:false,note:'',stories:[],opened:[],feedback:[],favourites:mergeFavourites(Object.values(records))}); }
  function status() {
    var text;
  ...
  function whyText(item) {
    ...
    if (!item.picked && item.spark !== REVIEW && nudgedToday(item.spark, records, day)) {
      var because = preferenceReason(item.spark, records, day);
  ...
  function render() {
    ...
    status(); renderHistory(); renderLibrary(); renderPersonal(); renderStart(); renderDiscovery();
  ...
  function chooseForIntention(intent) {
    ...
    candidates.sort(function(a,b) { return preferenceScore(b,records,day)-preferenceScore(a,records,day); });
