...
(function(root) {
  ...
  function render() {
    ...
    renderCompact(item, prompt);
    status(); renderHistory(); renderLibrary();
  }
  function renderCompact(item, prompt) {
    var compact = (item.done || item.tucked) && !expanded, note = item.note.trim().split('\n')[0];
    el('panel').className = 'daily-boost' + (compact ? ' is-compact' : '');
    el('compact-kicker').textContent = item.done ? '✓ Today’s spark · small win captured' : 'Today’s spark · tucked away for now';
  ...
  root.resetDailyBoost = function() { records = {}; account = null; day = null; expanded = false; if (el('note')) { el('note').value = ''; el('history').replaceChildren(); el('feedback').textContent = ''; } };
