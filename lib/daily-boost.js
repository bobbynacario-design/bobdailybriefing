...
(function(root) {
  ...
  function clean(value) {
    ...
    days.forEach(function(day) {
      ...
      entry.trialSource = typeof item.trialSource === 'string' ? item.trialSource.slice(0,4000) : '';
  ...
  root.createDailyBoostExperiment = function(action, due, source) {
    ...
    current().trialPlan = String(action).trim().slice(0,400); current().trialDue = due; current().trialSource = String(source || '').slice(0,4000); current().trialDone = false;
    persist(); render();
