...
(function(root) {
  ...
  // Unused sparks from an order position, in rotation order.
  function choicesFrom(position, saved, today, excluded) {
    var recent = {}, since = shiftDay(today, -RECENT_DAYS), choices = [];
    ...
    for (var step = 0; step < order.length; step++) {
      ...
      if (!recent[candidate] && candidate !== excluded) choices.push(candidate);
    }
    return choices;
  }
  // Feedback nudges the pick; it never takes the rotation over. Sorting the
  // whole order by preference let one "More like this" put its theme on every
  // other day for a month and drop two themes entirely. Now a theme Bob asked
  // for (a "More like this", or two "Helpful"s) comes back at most once every
  // FAVOURED_GAP days, as its next unused spark; every other day is the plain
  // rotation. A spark he said "Not today" to is passed over when it comes round.
  var NUDGE_MARGIN = 2, FAVOURED_GAP = 4;
  // The rotation's own pick: the first unused spark that was not turned down,
  // and not yesterday's theme (a favoured day can shift the rotation onto one).
  function rotationPick(choices, saved, today, lean) {
    var yesterday = saved && saved[shiftDay(today, -1)], avoid = yesterday ? themes[yesterday.spark] : null;
    var allowed = choices.filter(function(id) { return preferenceScore(id, saved, today, lean) >= 0; });
    return allowed.filter(function(id) { return themes[id] !== avoid; })[0] || allowed[0] || choices[0];
  }
  function pickFrom(position, saved, today, excluded, lean) {
    var choices = choicesFrom(position, saved, today, excluded);
    if (!choices.length) return order[position % order.length];
    var base = rotationPick(choices, saved, today, lean), lately = {};
    for (var back = 1; back < FAVOURED_GAP; back++) { var past = saved && saved[shiftDay(today, -back)]; if (past) lately[themes[past.spark]] = true; }
    var favoured = null, top = NUDGE_MARGIN - 1;
    choices.forEach(function(id) { var score = preferenceScore(id, saved, today, lean); if (score > top && !lately[themes[id]]) { favoured = id; top = score; } });
    return favoured || base;
  }
  // The latest response in the last 30 days that favours this spark's theme.
  function preferenceReason(spark, saved, today) {
    var keys = Object.keys(saved || {}).filter(function(key) {
      var response = saved[key].sparkResponse;
      return key < today && key >= shiftDay(today, -30) && response && response.value !== 'not-today' && themes[response.spark] === themes[spark];
    }).sort();
    if (!keys.length) return '';
    var key = keys[keys.length - 1], response = saved[key].sparkResponse, title = prompts[response.spark][0];
    return response.value === 'more' ? 'Because you asked for more like “' + title + '” on ' + label(key) : 'Because you found “' + title + '” helpful on ' + label(key);
  }
  // Whether today's rotation spark is there because of feedback (for the why line).
  function nudgedToday(spark, saved, today, lean) {
    var choices = choicesFrom(indexFor(today,0,order.length), saved, today);
    return !!choices.length && spark !== rotationPick(choices, saved, today, lean) && preferenceScore(spark, saved, today, lean) >= NUDGE_MARGIN;
  }
  // lean: the themes of Bob's goals, each worth as much as a "More like this".
  function preferenceScore(id, saved, today, lean) {
    var score = 0, previous = saved && saved[shiftDay(today,-1)];
    Object.keys(saved || {}).forEach(function(key) {
      var response = saved[key].sparkResponse;
      if (key >= today || key < shiftDay(today,-30) || !response) return;
      if (response.value === 'not-today') { if (response.spark === id) score -= 3; }
      else if (themes[response.spark] === themes[id]) score += response.value === 'more' ? 2 : 1;
    });
    if (lean && lean.indexOf(themes[id]) >= 0) score += NUDGE_MARGIN;
    // Keep variety, even when a theme has received lots of positive feedback.
    return previous && themes[previous.spark] === themes[id] ? Math.min(score,0) : Math.min(score,3);
  }
  function isSunday(day) { return new Date(day + 'T00:00:00Z').getUTCDay() === 0; }
  function sparkFor(day, saved, lean) {
    return isSunday(day) ? REVIEW : pickFrom(indexFor(day,0,order.length), saved, day, undefined, lean);
  }
  function nextSpark(from, saved, today, lean) {
    // Swapping away from the look-back resumes the rotation where today sits.
    var position = order.indexOf(from);
    return pickFrom(position < 0 ? indexFor(today,0,order.length) : position + 1, saved, today, from, lean);
  }
  ...
  // "More like this" / "Less like this" on briefing stories: vote 1, -1, or 0
  // (a vote taken back). The server turns recent votes into guidance for the
  // next briefing; they are not archived, since only recent ones matter.
  function cleanFeedback(list) {
    return (Array.isArray(list) ? list : []).filter(function(item) { return item && typeof item.headline === 'string' && item.headline.trim() && (item.vote === 1 || item.vote === -1 || item.vote === 0); }).slice(-30).map(function(item) {
      return {headline:item.headline.trim().slice(0,200), source:typeof item.source === 'string' ? item.source.slice(0,80) : '', section:typeof item.section === 'string' ? item.section.slice(0,20) : '',
        url:typeof item.url === 'string' && /^https?:\/\//i.test(item.url) ? item.url.slice(0,600) : '', vote:item.vote};
    });
  }
  ...
  // A check from the aha carries its whole wrong-if condition, so a little more
  // room than a briefing watch metric needs.
  var METRIC_CHARS = 340;
  // What an experiment was based on (a weekly read-back suggestion, say). Kept on
  // the day and in the yearly archive, so it is bounded like the other fields:
  // at 4,000 a year of daily use could push one archive doc towards the 1 MiB
  // Firestore limit, which would fail the save that retires days into it.
  var SOURCE_CHARS = 1500;
  function cleanReminders(list) {
    return (Array.isArray(list) ? list : []).filter(function(item) { return item && typeof item.metric === 'string' && item.metric.trim() && typeof item.due === 'string' && DAY_KEY.test(item.due); }).slice(-30).map(function(item) {
      return {metric:item.metric.trim().slice(0,METRIC_CHARS), headline:typeof item.headline === 'string' ? item.headline.slice(0,200) : '', source:typeof item.source === 'string' ? item.source.slice(0,80) : '',
        url:typeof item.url === 'string' && /^https?:\/\//i.test(item.url) ? item.url.slice(0,600) : '', due:item.due, done:item.done === true, doneOn:typeof item.doneOn === 'string' && DAY_KEY.test(item.doneOn) ? item.doneOn : '', finding:typeof item.finding === 'string' ? item.finding.slice(0,400) : ''};
    });
  }
  ...
  var GOAL_ID = /^g[a-z0-9]{2,20}$/;
  var FIELDS = ['energy','spark','done','tucked','note','carry','ref','refSource','refUrl','picked','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource','trialResult','trialNext','sparkResponse','resurfaced','goalTicks'];
  var TRIAL_RESULTS = {'':'Choose an outcome',helped:'Helped',mixed:'Mixed',unhelpful:'Didn’t help',untried:'Haven’t tried'};
  var TRIAL_NEXT = {'':'Choose a next step',keep:'Keep doing',adapt:'Adapt',release:'Let go'};
  ...
  var LISTS = ['stories','opened','reminders','feedback'];
  ...
  function clean(value) {
    ...
    days.forEach(function(day) {
      ...
      result[day] = {energy:item.energy === 'stretch' ? 'stretch' : 'gentle', offset:Number.isInteger(item.offset) && item.offset >= 0 ? item.offset % prompts.length : 0, done:item.done === true, tucked:item.tucked === true, note:typeof item.note === 'string' ? item.note.slice(0,1200) : '', carry:typeof item.carry === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(item.carry) ? item.carry : '', ref:typeof item.ref === 'string' ? item.ref.slice(0,300) : '', refSource:typeof item.refSource === 'string' ? item.refSource.slice(0,120) : '', refUrl:typeof item.refUrl === 'string' && /^https?:\/\//i.test(item.refUrl) ? item.refUrl.slice(0,600) : '', picked:item.picked === 'library' || item.picked === 'swap' || item.picked === 'intention' ? item.picked : '', stories:cleanStories(item.stories), reminders:cleanReminders(item.reminders), feedback:cleanFeedback(item.feedback), opened:day >= openedSince ? cleanOpened(item.opened) : [], updatedAt:Number.isFinite(item.updatedAt) && item.updatedAt > 0 ? Math.floor(item.updatedAt) : 0};
      ...
      entry.trialSource = typeof item.trialSource === 'string' ? item.trialSource.slice(0,SOURCE_CHARS) : '';
      ...
      entry.trialResult = Object.prototype.hasOwnProperty.call(TRIAL_RESULTS,item.trialResult) ? item.trialResult : '';
      entry.trialNext = Object.prototype.hasOwnProperty.call(TRIAL_NEXT,item.trialNext) ? item.trialNext : '';
      var response = item.sparkResponse;
      entry.goalTicks = Array.isArray(item.goalTicks) ? item.goalTicks.filter(function(id, i, all) { return typeof id === 'string' && GOAL_ID.test(id) && all.indexOf(id) === i; }).slice(0,3) : [];
      // A past discovery brought back that day ("note:<day>" or "trial:<day>") and what Bob chose.
      var back = item.resurfaced;
      entry.resurfaced = back && typeof back.ref === 'string' && /^(note|trial):\d{4}-\d{2}-\d{2}$/.test(back.ref) && ['','used','later','dismissed'].indexOf(back.action || '') >= 0 ? {ref:back.ref, action:back.action || ''} : null;
      entry.sparkResponse = response && Number.isInteger(response.spark) && response.spark >= 0 && response.spark < prompts.length && ['helpful','not-today','more'].indexOf(response.value) >= 0 ? {spark:response.spark,value:response.value,note:String(response.note || '').slice(0,240)} : null;
  ...
  // ── Archive ──
  // The account doc keeps the latest 90 days (clean), so a journal would lose
  // its oldest days. Days leaving that window are archived first, into one doc
  // per year, keeping what lasts (note, spark, stories, reminders, experiment,
  // carried note) and leaving out sync bookkeeping and reading history.
  var ARCHIVE_FIELDS = ['energy','spark','done','note','carry','ref','refSource','refUrl','stories','reminders','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource','trialResult','trialNext','sparkResponse','resurfaced','goalTicks','updatedAt'];
  function archiveEntry(day, value) {
    var entry = clean({[day]: value})[day];
    if (!entry) return null;
    var slim = {};
    ARCHIVE_FIELDS.forEach(function(field) { slim[field] = entry[field]; });
    return slim;
  }
  function worthKeeping(entry) {
    return !!entry && (won(entry) || entry.stories.length || entry.reminders.length || !!entry.trialPlan || !!entry.ref || !!entry.sparkResponse || !!(entry.resurfaced && entry.resurfaced.action) || entry.goalTicks.length > 0);
  }
  // {year: {day: entry}} for the days in `before` that fell out of the kept
  // window for age (older than its oldest day). Days removed for any other
  // reason, and days with nothing in them, are not archived.
  function retire(before, kept) {
    var out = {}, oldest = Object.keys(kept || {}).sort()[0] || '';
    Object.keys(before || {}).forEach(function(key) {
      if (!DAY_KEY.test(key) || (kept && kept[key]) || !oldest || key >= oldest) return;
      var entry = archiveEntry(key, before[key]);
      if (!worthKeeping(entry)) return;
      (out[key.slice(0,4)] = out[key.slice(0,4)] || {})[key] = entry;
    });
    return out;
  }
  // An archive as loaded (any number of days, each cleaned on its own).
  function cleanArchive(entries) {
    var out = {};
    Object.keys(entries || {}).forEach(function(key) {
      if (!DAY_KEY.test(key)) return;
      var entry = archiveEntry(key, entries[key]);
  ...
  root.DailyBoostCore = {dateKey:dateKey,promptFor:promptFor,sparkFor:sparkFor,nextSpark:nextSpark,clean:clean,merge:merge,retire:retire,cleanArchive:cleanArchive,parseDue:parseDue,review:REVIEW,linked:Object.keys(BRIEFING).map(Number),count:prompts.length,themes:themes.slice(),order:order.slice(),
  ...
  // archive: days older than the 90-day window (loaded once from the account
  // after the first sync, plus any the local window drops), read-only here.
  var archive = {}, archiveFor = null;
  function dayEntry(key) { return records[key] || archive[key] || null; }
  function allDays() { return Object.keys(Object.assign({}, archive, records)).sort(); }
  // Days the local window is about to drop stay reachable from the archive
  // (the account archives them too, in the save that drops them).
  function keepRetired(before, kept) {
    var dropped = retire(before, kept);
    Object.keys(dropped).forEach(function(year) { Object.assign(archive, dropped[year]); });
  }
  function loadArchive(uid) {
    if (typeof root.fbLoadDailyBoostArchive !== 'function') return;
    archiveFor = uid;
    root.fbLoadDailyBoostArchive(uid).then(function(entries) {
      if (uid !== account) return;
      var loaded = cleanArchive(entries);
      Object.keys(loaded).forEach(function(key) { if (!archive[key]) archive[key] = loaded[key]; });
      render();
    }, function() { if (uid === account) archiveFor = null; });
  }
  ...
  var sync = freshSync();
  function freshSync() { return {state:'local', loading:false, loadedFor:null, lastPull:0, timer:null, inflight:0, dirty:{}, stale:[], pending:new Set(), loadPromise:null}; }
  ...
  function current() { return records[day] || (records[day] = {energy:'gentle',spark:sparkFor(day,records,goalLean()),offset:0,done:false,note:'',stories:[],opened:[],feedback:[],favourites:mergeFavourites(Object.values(records))}); }
  function status() {
    var text;
  ...
  // ── The reflection's save state, beside the box ──
  // The note saves as Bob types (on the device at once, to the account 1.5 s
  // later), so there is no save button. This line says so where he is looking:
  // "Saving…", then "✓ Saved 9:41 PM · on all your devices", then it fades.
  // rev counts note edits this session; savedRev is the edit that line confirms.
  var noteEdit = {rev:0, savedRev:0, savedAt:0, fadedRev:0, fade:null};
  function resetNoteEdit() { if (noteEdit.fade) root.clearTimeout(noteEdit.fade); noteEdit = {rev:0, savedRev:0, savedAt:0, fadedRev:0, fade:null}; }
  function clockTime(t) {
    try { return new Date(t).toLocaleTimeString([], {hour:'numeric', minute:'2-digit'}); } catch (error) { return ''; }
  }
  function noteStatus() {
    var box = el('note-state');
    if (!box) return;
    var text = '', tone = '';
    if (!noteEdit.rev || !account) text = '';
    else if (failed) { text = 'Not saved on this device. Copy your note before leaving.'; tone = 'is-warn'; }
    // Once this edit is confirmed, a later background sync (another device's
    // days, a vote) is not the note saving again, so the line stays as it was.
    else if (linked() && sync.state === 'error' && noteEdit.savedRev !== noteEdit.rev) { text = 'Saved on this device · will reach your account when it is back'; tone = 'is-warn'; }
    else if (!linked() || sync.state === 'synced' || noteEdit.savedRev === noteEdit.rev) {
      if (noteEdit.savedRev !== noteEdit.rev) { noteEdit.savedRev = noteEdit.rev; noteEdit.savedAt = Date.now(); flashNoteSaved(); }
      var at = clockTime(noteEdit.savedAt);
      text = '✓ Saved' + (at ? ' ' + at : '') + (linked() ? ' · on all your devices' : ' · on this device');
      tone = 'is-saved' + (noteEdit.fadedRev === noteEdit.rev ? ' is-faded' : '');
    }
    else text = 'Saving…';
    box.textContent = text;
    box.className = 'boost-note-state' + (tone ? ' ' + tone : '');
  }
  // Show the saved line for four seconds, then fade it (a newer edit cancels this).
  function flashNoteSaved() {
    var rev = noteEdit.rev;
    noteEdit.fadedRev = 0;
    if (noteEdit.fade) root.clearTimeout(noteEdit.fade);
    noteEdit.fade = root.setTimeout(function() { noteEdit.fade = null; if (noteEdit.savedRev === rev && noteEdit.rev === rev) { noteEdit.fadedRev = rev; noteStatus(); } }, 4000);
  }
  function noteTyped() {
    noteEdit.rev++;
    if (el('note-done')) el('note-done').hidden = false;
  }
  // Done: nothing to save that is not saved already, so it sends a pending edit
  // now rather than in 1.5 s, closes the keyboard, and shows the confirmation.
  function noteDone() {
    if (sync.timer) push();
    else if (noteEdit.rev && noteEdit.savedRev === noteEdit.rev) flashNoteSaved();
    var note = el('note');
    if (note && note.blur) note.blur();
    if (el('note-done')) el('note-done').hidden = true;
    status();
  }
  function saveLocal() {
    var cleaned = clean(records);
    keepRetired(records, cleaned);
    records = cleaned;
  ...
  function whyText(item) {
    ...
    // Named only when feedback actually moved the rotation's pick, and by the
    // response that did it, not a generic "you liked this theme".
    if (!item.picked && item.spark !== REVIEW && nudgedToday(item.spark, records, day, goalLean())) {
      var because = preferenceReason(item.spark, records, day) || goalReason(item.spark);
  ...
  function pull(minGap) {
    if (sync.loading) return sync.loadPromise;
    if (!linked() || Date.now() - sync.lastPull < (minGap === undefined ? 60000 : minGap)) return;
    var uid = account, session = sync;
    sync.loading = true; sync.lastPull = Date.now();
    if (sync.state !== 'synced') sync.state = 'syncing';
    status();
    sync.loadPromise = root.fbLoadDailyBoost(uid).then(function(entries) {
      ...
      keepRetired(Object.assign({}, entries, records), result.records);
      if (archiveFor !== uid) loadArchive(uid);
  ...
  function push() {
    ...
    var task = root.fbSaveDailyBoost(uid, entries, stale).then(function(confirmed) {
      if (uid !== account || session !== sync) return;
      if (confirmed) {
    ...
    task.then(function() { session.pending.delete(task); },function() { session.pending.delete(task); });
    return task;
  }
  // Generation reads the account copy. Await every edit, including a save
  // already in flight, and stop rather than generate from an unsaved draft.
  root.flushDailyBoost = async function() {
    if (!linked()) throw new Error('Connect to your account before reading your week. Your notes remain on this device.');
    var session = sync, uid = account;
    function waitForSave(promise) {
      return new Promise(function(resolve,reject) {
        var timer = root.setTimeout(function() { reject(new Error('Saving is taking too long. Reconnect and try reading your week again.')); },20000);
        Promise.resolve(promise).then(function(value) { root.clearTimeout(timer); resolve(value); },function(error) { root.clearTimeout(timer); reject(error); });
      });
    }
    if (sync.loadedFor !== account || sync.loading) await waitForSave(pull(0));
    for (var attempt = 0; attempt < 6; attempt++) {
      if (sync !== session || account !== uid || root._firebaseUid !== uid) throw new Error('Account changed. Please try again.');
      if (sync.loadedFor !== uid || sync.state === 'error') throw new Error('Your latest notes could not sync. Reconnect and try again before reading your week.');
      await waitForSave(Promise.all(Array.from(session.pending)));
      if (sync !== session || account !== uid || root._firebaseUid !== uid) throw new Error('Account changed. Please try again.');
      if (sync.state === 'error') throw new Error('Your latest notes could not sync. Reconnect and try again before reading your week.');
      if (Object.keys(sync.dirty).length || sync.stale.length) { await waitForSave(push()); continue; }
      if (!sync.inflight) return;
    }
    throw new Error('Notes are still changing. Finish your note, then read your week again.');
  };
  ...
  function renderHistory() {
    ...
    if (shownDay && dayEntry(shownDay) && shownDay < day) days = [shownDay].concat(days.filter(function(key) { return key !== shownDay; }));
    var archived = Object.keys(archive).filter(function(key) { return !records[key]; }).length;
    el('history-title').textContent = (days.length ? 'Your recent discoveries (' + days.length + ')' : 'Your recent discoveries') + (archived ? ' · ' + archived + ' older in your archive' : '');
    ...
    days.forEach(function(key) {
      ...
      var past = dayEntry(key);
      heading.textContent = label(key) + (key.slice(0,4) !== day.slice(0,4) ? ' ' + key.slice(0,4) : '') + ' · ' + prompts[past.spark][0] + (past.done ? ' · Done' : '');
      note.textContent = past.note || 'Made time for a small quest.';
  ...
  function render() {
    ...
    if (switched) {
      goalsFor = null;
      account = uid; records = {}; baseline = {}; syncBase = {}; sentNotes = {}; archive = {}; archiveFor = null; failed = false; shownDay = null; resetSync(); resetNoteEdit();
      if (el('note-done')) el('note-done').hidden = true;
    ...
    status(); renderHistory(); renderLibrary(); renderPersonal(); renderStart(); renderDiscovery(); renderGoals();
  ...
  // ── Start here: the one next step on Today ──
  // Today grew to seven things asking for attention (the spark, experiments, To
  // check, the aha, the weekly read, the briefing, the note). This names one, in
  // order of what will not wait: a check that is due, a goal move due today, an
  // experiment ready to review, the Sunday read, today's spark, then a line in the note. "Not now"
  // moves on for the rest of this visit; nothing else on the page is hidden.
  var START_NAMES = {check:'a check', review:'an experiment review', goal:'a goal’s next move', read:'your weekly read', spark:'today’s spark', note:'a line in your note'};
  var startSkipped = {day:'', kinds:{}}, startKind = '';
  function startSteps() {
    var item = current(), steps = [];
    var due = allReminders().filter(function(entry) { return !entry.item.done && entry.item.due <= day; });
    if (due.length) steps.push({kind:'check', kicker:'Start here · ' + (due.length === 1 ? 'a check is due' : due.length + ' checks are due'), main:due[0].item.metric, go:'Open it ↓'});
    var dueGoals = activeGoals().filter(function(goal) { return goal.nextMove && goal.nextDue && goal.nextDue <= day; }).sort(function(a,b) { return a.nextDue.localeCompare(b.nextDue); });
    if (dueGoals.length) steps.push({kind:'goal', kicker:'Start here · a goal move is due', main:dueGoals[0].nextMove + ' · ' + dueGoals[0].text, go:'Open goal ↓'});
    var ready = Object.keys(records).filter(function(key) { var entry = records[key]; return entry.trialPlan && !entry.trialDone && entry.trialDue && entry.trialDue <= day; }).sort();
    if (ready.length) steps.push({kind:'review', kicker:'Start here · time to review', main:'Did “' + records[ready[0]].trialPlan + '” help?', go:'Review ↓'});
    if (weeklyReadWaiting()) steps.push({kind:'read', kicker:'Start here · it’s Sunday', main:'Read your week back, then carry one thing forward.', go:'Read my week ↓'});
    if (!item.done && !item.tucked) steps.push({kind:'spark', kicker:'Start here · today’s spark', main:prompts[item.spark][0] + ' · ' + (item.energy === 'stretch' ? '10 minutes' : '2 minutes'), go:'Start ↓'});
    if (!item.note.trim()) steps.push({kind:'note', kicker:'Start here · one line', main:'What clicked for you today?', go:'Write it ↓'});
    return steps;
  }
  function renderStart() {
    var box = el('start');
    if (!box) return;
    if (!account) { box.hidden = true; startKind = ''; return; }
    if (startSkipped.day !== day) startSkipped = {day:day, kinds:{}};
    var steps = startSteps(), open = steps.filter(function(step) { return !startSkipped.kinds[step.kind]; });
    var step = open[0] || {kind:'done', kicker:'All done for today ✓', main:steps.length ? 'The rest can wait. The briefing is below.' : 'Nothing waiting. The briefing is below.', go:'To the briefing ↓'};
    startKind = step.kind; box.hidden = false;
    box.className = 'start-here' + (step.kind === 'done' ? ' is-done' : '');
    // A check can run to 340 characters; the card stays a glance, the full text is one tap away.
    var main = step.main.length > 120 ? step.main.slice(0,119).replace(/\s+\S*$/, '') + '…' : step.main;
    el('start-kicker').textContent = step.kicker; el('start-main').textContent = main; el('start-go').textContent = step.go;
    if (el('start-main').setAttribute) el('start-main').setAttribute('title', step.main);
    var rest = open.slice(1).map(function(other) { return START_NAMES[other.kind]; });
    el('start-then').textContent = rest.length ? 'Then: ' + rest.join(' · ') : '';
    el('start-skip').hidden = step.kind === 'done';
  }
  function jumpTo(node) {
    if (!node) return;
    if (node.setAttribute && node.getAttribute && !node.getAttribute('tabindex')) node.setAttribute('tabindex','-1');
    if (node.scrollIntoView) node.scrollIntoView({block:'start', behavior:'smooth'});
    if (node.focus) node.focus({preventScroll:true});
  }
  function startGo() {
    if (startKind === 'check') jumpTo(el('reminders'));
    else if (startKind === 'review') jumpTo(el('experiments-panel'));
    else if (startKind === 'goal') { el('goals-panel').open = true; jumpTo(el('goals-panel')); }
    else if (startKind === 'read') openWeeklyRead();
    else if (startKind === 'spark') { reopen(); if (el('title').scrollIntoView) el('title').scrollIntoView({block:'center', behavior:'smooth'}); }
    else if (startKind === 'note') focusNote();
    else jumpTo(document.getElementById('daily-intelligence'));
  }
  // Open the weekly read (it lives inside the expanded panel) and go to it.
  function openWeeklyRead() {
    expanded = true; render();
    if (el('mirror-panel')) { el('mirror-panel').open = true; jumpTo(el('mirror-panel')); }
  }
  function weeklyReadWaiting() { return isSunday(day) && typeof root.weeklyMirrorReadOn === 'function' && !root.weeklyMirrorReadOn(day); }
  function startSkip() {
    if (!startKind || startKind === 'done') return;
    startSkipped.kinds[startKind] = true; renderStart();
  }
  // ── Your goals: one to three things Bob is working towards ──
  // They live on their own account doc (briefings-bob/goals-<uid>, via
  // fbLoadGoals / fbSaveGoals) with a device copy for a fast first paint. Each
  // has a theme; the rotation leans towards those themes on the same gentle
  // terms as "More like this" (goalLean). Each day Bob can tick the goals he
  // moved (goalTicks on the day), and the panel counts them over two weeks.
  // The weekly read measures the week against them (functions/weekly-mirror.js).
  var GOAL_THEMES = [['Momentum','Momentum · getting going'],['Craft','Work craft'],['Perspective','Perspective'],['Curiosity','Curiosity'],['Creativity','Creativity'],['Confidence','Confidence'],['Connection','Connection'],['Reset','Rest & reset']];
  var goals = [], goalsFor = null, goalsEditing = false, goalsMessage = '';
  function goalThemeOk(theme) { return GOAL_THEMES.some(function(pair) { return pair[0] === theme; }); }
  function cleanGoals(list) {
    var seen = {};
    return (Array.isArray(list) ? list : []).filter(function(goal) {
      if (!goal || typeof goal.id !== 'string' || !GOAL_ID.test(goal.id) || seen[goal.id] || !String(goal.text || '').trim()) return false;
      seen[goal.id] = true; return true;
    }).slice(0, 12).map(function(goal) {
      var due = String(goal.nextDue || '');
      return {id:goal.id, text:String(goal.text).trim().slice(0,120), theme:goalThemeOk(goal.theme) ? goal.theme : 'Momentum', archived:goal.archived === true, createdAt:typeof goal.createdAt === 'string' ? goal.createdAt.slice(0,30) : '',
        nextMove:String(goal.nextMove || '').trim().slice(0,200), nextDue:DAY_KEY.test(due) && Number.isFinite(Date.parse(due + 'T00:00:00Z')) ? due : ''};
    });
  }
  function activeGoals() { return goals.filter(function(goal) { return !goal.archived; }).slice(0,3); }
  function goalLean() { return activeGoals().map(function(goal) { return goal.theme; }); }
  function goalReason(spark) {
    var goal = activeGoals().filter(function(one) { return one.theme === themes[spark]; })[0];
    return goal ? 'For your goal “' + goal.text + '”' : '';
  }
  // A theme to start from, guessed from the words; Bob can change it.
  function guessTheme(text) {
    var t = String(text || '').toLowerCase();
    var rules = [[/consisten|habit|routine|daily|disciplin|start|momentum|finish/,'Momentum'],[/client|consult|offer|business|advis|report|forensic|claim|skill|career|work/,'Craft'],
      [/understand|myself|reflect|clarity|perspective|think/,'Perspective'],[/learn|curious|explore|read|study/,'Curiosity'],[/creat|write|build|design|idea|make/,'Creativity'],
      [/confiden|speak|lead|brave|courage/,'Confidence'],[/people|network|family|friend|connect|relationship|team/,'Connection'],[/rest|calm|sleep|health|stress|balance|energy/,'Reset']];
    for (var i = 0; i < rules.length; i++) if (rules[i][0].test(t)) return rules[i][1];
    return 'Momentum';
  }
  function saveGoalsLocal() { try { root.localStorage.setItem('bob-goals:' + account, JSON.stringify(goals)); } catch (error) {} }
  function loadGoals() {
    if (!account || goalsFor === account) return;
    var uid = account;
    goalsFor = uid; goalsEditing = false; goalsMessage = '';
    try { goals = cleanGoals(JSON.parse(root.localStorage.getItem('bob-goals:' + uid) || '[]')); } catch (error) { goals = []; }
    if (typeof root.fbLoadGoals === 'function') {
      root.fbLoadGoals(uid).then(function(remote) {
        if (uid !== account || !Array.isArray(remote)) return;
        goals = cleanGoals(remote); saveGoalsLocal(); render();
      }, function() {});
    }
  }
  function saveGoals(next) {
    var uid = account;
    goals = cleanGoals(next); saveGoalsLocal();
    goalsMessage = 'Saved.';
    if (typeof root.fbSaveGoals === 'function') {
      root.fbSaveGoals(uid, goals).then(function() {}, function() {
        if (uid !== account) return;
        goalsMessage = 'Saved on this device; your account copy will catch up when you are back online.'; render();
      });
    }
    render();
  }
  function toggleGoalTick(id) {
    render();
    if (!account || !activeGoals().some(function(goal) { return goal.id === id; })) return;
    var ticks = current().goalTicks || (current().goalTicks = []), at = ticks.indexOf(id);
    if (at >= 0) ticks.splice(at, 1); else ticks.push(id);
    persist(); render();
  }
  function finishGoalMove(id) {
    var goal = activeGoals().filter(function(item) { return item.id === id; })[0];
    if (!account || !goal || !goal.nextMove) return;
    var ticks = current().goalTicks || (current().goalTicks = []);
    if (ticks.indexOf(id) < 0) ticks.push(id);
    goal.nextMove = ''; goal.nextDue = '';
    persist();
    goalsEditing = true;
    saveGoals(goals);
    goalsMessage = 'Move captured. Add the next one while it is clear.'; render();
  }
  function goalDays(id, days) {
    var out = [];
    for (var i = days - 1; i >= 0; i--) { var key = shiftDay(day, -i), entry = dayEntry(key); out.push({key:key, moved:!!entry && (entry.goalTicks || []).indexOf(id) >= 0}); }
    return out;
  }
  function renderGoals() {
    var panel = el('goals-panel');
    if (!panel) return;
    var list = activeGoals(), item = current();
    el('goals-summary').textContent = list.length ? 'Your goals · ' + list.length : 'Your goals';
    // Today's ticks, under the quest.
    var ticks = el('goal-tick-list');
    ticks.replaceChildren();
    el('goal-ticks').hidden = !account || !list.length;
    list.forEach(function(goal) {
      var chip = document.createElement('button'), on = (item.goalTicks || []).indexOf(goal.id) >= 0;
      chip.type = 'button'; chip.className = 'tool-chip'; chip.setAttribute('aria-pressed', String(on));
      chip.textContent = (on ? '✓ ' : '') + (goal.text.length > 34 ? goal.text.slice(0,33) + '…' : goal.text);
      chip.title = on ? 'Marked as moved today. Tap to undo.' : 'Tap if today moved this goal';
      chip.addEventListener('click', function() { toggleGoalTick(goal.id); });
      ticks.appendChild(chip);
    });
    // The list with its two-week record, or the editor.
    var box = el('goals-list'), editor = el('goals-editor');
    box.replaceChildren(); editor.replaceChildren();
    box.hidden = goalsEditing; editor.hidden = !goalsEditing;
    el('goals-edit').hidden = goalsEditing || !account;
    el('goals-edit').textContent = list.length ? 'Edit goals' : 'Set your goals';
    el('goals-status').textContent = !account ? 'Sign in to set goals.' : goalsMessage;
    if (!goalsEditing) {
      if (!list.length) { box.textContent = 'No goals yet. Name one to three things you are working towards; the app will start leaning towards them.'; return; }
      list.forEach(function(goal) {
        var row = document.createElement('article'), title = document.createElement('h4'), meta = document.createElement('p'), dots = document.createElement('div');
        var record = goalDays(goal.id, 14), moved = record.filter(function(d) { return d.moved; }).length;
        title.textContent = goal.text;
        meta.className = 'boost-kicker';
        meta.textContent = (GOAL_THEMES.filter(function(pair) { return pair[0] === goal.theme; })[0] || [0,''])[1] + ' · moved on ' + moved + (moved === 1 ? ' day' : ' days') + ' of the last 14';
        dots.className = 'boost-goal-dots'; dots.setAttribute('aria-label', 'Moved on ' + moved + ' of the last 14 days');
        record.forEach(function(d) { var dot = document.createElement('span'); dot.className = 'boost-dot' + (d.moved ? ' is-won' : '') + (d.key === day ? ' is-today' : ''); dot.title = label(d.key) + (d.moved ? ' · moved' : ''); dots.appendChild(dot); });
        row.append(title, meta);
        if (goal.nextMove) {
          var move = document.createElement('p'), done = document.createElement('button');
          move.className = 'boost-goal-next'; move.textContent = 'Next: ' + goal.nextMove + (goal.nextDue ? ' · ' + (goal.nextDue <= day ? 'due ' : 'by ') + label(goal.nextDue) : '');
          done.type = 'button'; done.className = 'tool-chip'; done.textContent = 'Done · set next move';
          done.addEventListener('click', function() { finishGoalMove(goal.id); });
          row.append(move, done);
        } else {
          var missing = document.createElement('p'); missing.className = 'boost-goal-next'; missing.textContent = 'No next move set.'; row.appendChild(missing);
        }
        row.append(dots); box.appendChild(row);
      });
      return;
    }
    var rows = list.slice();
    while (rows.length < 3) rows.push({id:'', text:'', theme:''});
    var inputs = rows.map(function(goal, index) {
      var row = document.createElement('div'), text = document.createElement('input'), move = document.createElement('input'), due = document.createElement('input'), theme = document.createElement('select');
      row.className = 'boost-goal-edit';
      text.type = 'text'; text.maxLength = 120; text.value = goal.text; text.placeholder = index ? 'Another one (optional)' : 'Something you are working towards';
      text.setAttribute('aria-label', 'Goal ' + (index + 1));
      move.type = 'text'; move.maxLength = 200; move.value = goal.nextMove || ''; move.placeholder = 'Next concrete move (optional)'; move.setAttribute('aria-label','Next move for goal ' + (index + 1));
      due.type = 'date'; due.value = goal.nextDue || ''; due.min = day; due.setAttribute('aria-label','Target date for goal ' + (index + 1));
      GOAL_THEMES.forEach(function(pair) { var option = document.createElement('option'); option.value = pair[0]; option.textContent = pair[1]; theme.appendChild(option); });
      theme.value = goal.theme || guessTheme(goal.text); theme.setAttribute('aria-label', 'Theme for goal ' + (index + 1));
      var chosen = !!goal.theme;
      theme.addEventListener('change', function() { chosen = true; });
      text.addEventListener('input', function() { if (!chosen) theme.value = guessTheme(text.value); });
      row.append(text, move, due, theme); editor.appendChild(row);
      return {goal:goal, text:text, move:move, due:due, theme:theme};
    });
    var actions = document.createElement('div'), save = document.createElement('button'), cancel = document.createElement('button');
    actions.className = 'boost-goals-actions';
    save.type = 'button'; save.className = 'tool-chip'; save.textContent = 'Save goals';
    cancel.type = 'button'; cancel.className = 'tool-chip'; cancel.textContent = 'Cancel';
    save.addEventListener('click', function() {
      var kept = [], now = new Date().toISOString();
      inputs.forEach(function(input) {
        var textValue = input.text.value.trim();
        if (!textValue) return;
        kept.push({id:input.goal.id || 'g' + Date.now().toString(36) + kept.length, text:textValue, theme:input.theme.value, archived:false, createdAt:input.goal.createdAt || now,
          nextMove:input.move.value.trim().slice(0,200), nextDue:input.move.value.trim() && DAY_KEY.test(input.due.value) ? input.due.value : ''});
      });
      // A goal cleared from the editor is archived, so its ticks keep their meaning.
      var archived = goals.filter(function(goal) { return goal.archived || !kept.some(function(k) { return k.id === goal.id; }); }).map(function(goal) { return Object.assign({}, goal, {archived:true}); });
      goalsEditing = false;
      saveGoals(kept.concat(archived));
    });
    cancel.addEventListener('click', function() { goalsEditing = false; goalsMessage = ''; render(); });
    actions.append(save, cancel); editor.appendChild(actions);
  }
  // The oldest open experiment whose revisit date has come, for the Morning 5
  // (functions/delivery-core.js dueExperiment picks the same one for the push).
  root.dailyBoostDueExperiment = function() {
    if (!account) return null;
    var key = Object.keys(records).filter(function(k) { var entry = records[k]; return entry.trialPlan && !entry.trialDone && entry.trialDue && entry.trialDue <= day; })
      .sort(function(a, b) { return records[a].trialDue.localeCompare(records[b].trialDue) || a.localeCompare(b); })[0];
    return key ? {plan:records[key].trialPlan, due:records[key].trialDue, day:key} : null;
  };
  root.dailyBoostGoals = function() { return activeGoals().map(function(goal) { return {id:goal.id, text:goal.text, theme:goal.theme, nextMove:goal.nextMove, nextDue:goal.nextDue}; }); };

  // ── From your past: one of Bob's own discoveries, brought back ──
  // Every third day (never on the Sunday look-back), a note he wrote at least two
  // weeks ago, or an experiment at least a week old that he said helped or wanted
  // to keep or adapt, comes back with "Still useful?". Use today puts it to work
  // (the note in today's reflection, the plan in today's experiment box); Later
  // brings it back in about two weeks; Dismiss retires it. The pick and the
  // choice are kept on the day ("resurfaced"), so every device agrees, and
  // anything shown rests for a month.
  var RESURFACE_EVERY = 3, NOTE_AGE = 14, TRIAL_AGE = 7, pastMessage = '';
  var STUB_LINE = /^(On “.+”(?: \([^()]*\))?|My take on “.+”):\s*$/;
  function ownWords(note) {
    return String(note || '').split(/\n+/).map(function(line) { return line.trim(); }).filter(function(line) { return line && !STUB_LINE.test(line); }).join(' ');
  }
  function dayNumber(key) { return Math.floor(Date.parse(key + 'T00:00:00Z') / 86400000); }
  function discoveryFrom(ref) {
    var match = /^(note|trial):(.+)$/.exec(ref || ''), entry = match && dayEntry(match[2]);
    if (!entry) return null;
    return match[1] === 'note' ? {ref:ref, day:match[2], kind:'note', text:ownWords(entry.note)} : {ref:ref, day:match[2], kind:'trial', text:entry.trialPlan, result:entry.trialResult, next:entry.trialNext};
  }
  // When each discovery was last shown and what was chosen; a dismissal sticks.
  function resurfaceHistory() {
    var seen = {};
    allDays().forEach(function(key) {
      var back = key !== day && dayEntry(key) && dayEntry(key).resurfaced;
      if (!back) return;
      var info = seen[back.ref] || (seen[back.ref] = {});
      info.shown = key; info.action = back.action;
      if (back.action === 'dismissed') info.dismissed = true;
    });
    return seen;
  }
  function rested(info) {
    if (!info) return true;
    if (info.dismissed) return false;
    return info.shown <= shiftDay(day, -(info.action === 'used' ? 60 : info.action === 'later' ? 14 : 30));
  }
  function resurfaceCandidates() {
    var seen = resurfaceHistory(), out = [];
    allDays().forEach(function(key) {
      var entry = dayEntry(key);
      if (!entry || key >= day) return;
      if (key <= shiftDay(day, -NOTE_AGE) && ownWords(entry.note).length >= 25 && rested(seen['note:' + key])) out.push(discoveryFrom('note:' + key));
      if (entry.trialPlan && key <= shiftDay(day, -TRIAL_AGE) && (entry.trialResult === 'helped' || entry.trialNext === 'keep' || entry.trialNext === 'adapt') && rested(seen['trial:' + key])) out.push(discoveryFrom('trial:' + key));
    });
    return out;
  }
  function todaysDiscovery() {
    if (!account) return null;
    var item = current();
    if (item.resurfaced) return discoveryFrom(item.resurfaced.ref);
    if (isSunday(day) || dayNumber(day) % RESURFACE_EVERY !== 0) return null;
    var list = resurfaceCandidates();
    if (!list.length) return null;
    var pick = list[Math.floor(dayNumber(day) / RESURFACE_EVERY) % list.length];
    item.resurfaced = {ref:pick.ref, action:''}; // saved with the day's next write
    return pick;
  }
  function ago(key) {
    var days = dayNumber(day) - dayNumber(key);
    return days < 14 ? days + ' days ago' : days < 60 ? Math.round(days / 7) + ' weeks ago' : Math.round(days / 30) + ' months ago';
  }
  function clipped(value, max) { return value.length > max ? value.slice(0, max - 1).replace(/\s+\S*$/, '') + '…' : value; }
  function renderDiscovery() {
    var box = el('past');
    if (!box) return;
    var found = todaysDiscovery(), back = found && current().resurfaced;
    if (!found || !back) { box.hidden = true; pastMessage = ''; return; }
    var chosen = !!back.action;
    box.hidden = chosen && !pastMessage;
    el('past-kicker').textContent = 'From your past · ' + ago(found.day);
    el('past-text').textContent = found.kind === 'note' ? '“' + clipped(found.text, 240) + '”' : found.text;
    el('past-sub').textContent = found.kind === 'note' ? 'You wrote this on ' + label(found.day) + '. Still useful?'
      : 'An experiment from ' + label(found.day) + (found.result === 'helped' ? ' that you said helped' : found.next === 'adapt' ? ' you wanted to adapt' : ' you wanted to keep doing') + '. Still useful?';
    el('past-use').textContent = found.kind === 'note' ? 'Use today' : 'Try it again today';
    el('past-actions').hidden = chosen;
    el('past-status').textContent = pastMessage;
  }
  function discoveryAct(action) {
    render();
    var found = todaysDiscovery();
    if (!found) return;
    current().resurfaced = {ref:found.ref, action:action};
    if (action === 'used' && found.kind === 'trial') {
      if (current().trialPlan) { current().resurfaced.action = ''; pastMessage = 'Today already has an experiment, so this one waits for another day.'; persist(); render(); return; }
      pastMessage = 'It is in today’s experiment box below.';
      persist(); render();
      // After the render: it re-syncs an unfocused box from today's (empty) plan.
      el('trial-plan').value = found.text; el('trial-due').value = shiftDay(day, 3); el('trial-editor').open = true;
      el('trial-status').textContent = 'Brought back from ' + label(found.day) + '. Adjust it if you like, then save.';
      if (el('trial-plan').focus) el('trial-plan').focus();
      if (el('trial-plan').scrollIntoView) el('trial-plan').scrollIntoView({block:'center', behavior:'smooth'});
      return;
    }
    if (action === 'used') {
      pastMessage = 'Brought into today’s note.';
      var quote = clipped(found.text, 160);
      prefillNote('Coming back to “' + quote + '” (' + label(found.day) + '): ', quote);
      return;
    }
    pastMessage = action === 'later' ? 'It will come back in about two weeks.' : 'It won’t come back.';
    persist(); render();
  }
  ...
  function renderCompact(item, prompt, wins) {
    ...
    // The weekly read sits inside the panel this strip hides, so on Sundays the
    // strip offers it until it has been read.
    if (el('compact-week')) el('compact-week').hidden = !weeklyReadWaiting();
  }
  ...
  function exportText() {
    var days = allDays().filter(function(key) { var entry = dayEntry(key); return won(entry) || entry.trialPlan || entry.sparkResponse || (entry.reminders || []).length; }).reverse();
    ...
    days.forEach(function(key) {
      ...
      if (entry.trialSource) lines.push('Based on: ' + entry.trialSource);
      if (entry.trialResult) lines.push('Outcome: ' + TRIAL_RESULTS[entry.trialResult]);
      if (entry.trialNext) lines.push('Next step: ' + TRIAL_NEXT[entry.trialNext]);
      if (entry.sparkResponse) lines.push('Spark feedback (' + prompts[entry.sparkResponse.spark][0] + '): ' + entry.sparkResponse.value + (entry.sparkResponse.note ? ' — ' + entry.sparkResponse.note : ''));
      (entry.reminders || []).forEach(function(check) { if (check.finding) lines.push('Finding on ' + check.metric + ': ' + check.finding); });
  ...
  function renderLibrary() {
    ...
    prompts.forEach(function(prompt,index) {
      ...
      favourite.textContent = selected ? '★ Saved' : '☆ Save'; favourite.setAttribute('aria-pressed',String(selected));
  ...
  function chooseForIntention(intent) {
    ...
    if (!allowed) return nextSpark(current().spark,records,day,goalLean());
    var candidates = order.filter(function(id) { return allowed.indexOf(themes[id]) >= 0 && id !== current().spark; });
    candidates.sort(function(a,b) { return preferenceScore(b,records,day,goalLean())-preferenceScore(a,records,day,goalLean()); });
  ...
  function renderPersonal() {
    var item = current();
    var response = item.sparkResponse && item.sparkResponse.spark === item.spark ? item.sparkResponse : null;
    ['helpful','not-today','more'].forEach(function(value) {
      el('rate-' + value).setAttribute('aria-pressed',String(!!response && response.value === value));
    });
    el('rate-note').hidden = !response;
    if (document.activeElement !== el('rate-note')) el('rate-note').value = response ? response.note : '';
    el('intention').value = item.intention || ''; el('intention').disabled = item.done;
    el('intention-hint').textContent = item.done ? 'Undo today’s completion to change it.' : '';
    var fav = !!(favourites()[item.spark] || {}).on;
    el('favourite').textContent = fav ? '★ Saved' : '☆ Save';
    el('favourite').title = fav ? 'Saved to your favourites. Tap to remove.' : 'Save this spark to your favourites';
    ...
    list.replaceChildren();
    // Open experiments, plus reviewed ones from the last two weeks; older reviews
    // live on in the copied reflections.
    var keys = Object.keys(records).filter(function(key) { return records[key].trialPlan && (!records[key].trialDone || key >= shiftDay(day, -14)); }).sort(function(a,b) {
    ...
    keys.forEach(function(key) {
      ...
      button.addEventListener('click',function() {
        ...
        if (!records[key].trialDone && !records[key].trialResult && !records[key].trialOutcome.trim()) { when.textContent = 'Choose an outcome or write what happened before finishing.'; return; }
        records[key].trialDone = !records[key].trialDone; commitDays([key]);
      });
      row.append(title,when,outcomeLabel); list.appendChild(row);
      var choices = document.createElement('div'); choices.className = 'boost-library-controls';
      [['trialResult','How did it go?',TRIAL_RESULTS],['trialNext','What next?',TRIAL_NEXT]].forEach(function(spec) {
        var label = document.createElement('label'), select = document.createElement('select'); label.textContent = spec[1];
        Object.keys(spec[2]).forEach(function(value) { var option = document.createElement('option'); option.value = value; option.textContent = spec[2][value]; select.appendChild(option); });
        select.value = trial[spec[0]] || '';
        select.addEventListener('change',function() { if (!records[key]) return; records[key][spec[0]] = select.value; commitDays([key]); });
        label.appendChild(select); choices.appendChild(label);
      });
      row.append(choices,button);
      if (trial.trialNext === 'keep' || trial.trialNext === 'adapt') {
        var reuse = document.createElement('button'); reuse.className = 'tool-chip'; reuse.textContent = trial.trialNext === 'adapt' ? 'Adapt for today' : 'Try again today';
        reuse.addEventListener('click',function() {
          if (current().trialPlan) { when.textContent = 'Today already has an experiment. Your existing plan is kept.'; return; }
          el('trial-plan').value = trial.trialPlan;
          el('trial-due').value = shiftDay(day,3);
          el('trial-editor').open = true; expanded = true; el('panel').classList.remove('is-compact');
          el('trial-plan').focus(); el('trial-plan').scrollIntoView({block:'center',behavior:'smooth'});
          el('trial-status').textContent = 'Edit this plan if needed, then save your new experiment.';
        });
        row.appendChild(reuse);
      }
      if (trial.trialSource) {
        var provenance = document.createElement('details'), summary = document.createElement('summary'), text = document.createElement('p');
        summary.textContent = 'Based on'; text.textContent = trial.trialSource;
        provenance.append(summary,text); row.appendChild(provenance);
      }
    });
  }
  ...
  root.dailyBoostSearchEntries = function() {
    if (!account) return [];
    return allDays().filter(function(key) {
      var entry = dayEntry(key);
  ...
  root.openDailyBoostDay = function(key) {
    if (!el('note') || !dayEntry(key)) return false;
  ...
  // The latest vote on a story over the last 30 days: 1, -1, or 0.
  function voteFor(story) {
    if (!account || !day || !story) return 0;
    var id = storyKey(story);
    for (var i = 0; i < 30; i++) {
      var entry = records[shiftDay(day, -i)], mine = entry && (entry.feedback || []).filter(function(item) { return storyKey(item) === id; }).pop();
      if (mine) return mine.vote;
    }
    return 0;
  }
  root.dailyBoostVoteFor = voteFor;
  // More like this (1) / Less like this (-1); the same button again takes the
  // vote back. Kept on today, so the newest vote on a story is the one that counts.
  root.voteBriefingStory = function(story, vote) {
    if (!story || !story.headline || (vote !== 1 && vote !== -1) || !el('note')) return false;
    render();
    if (!account) return false;
    var next = voteFor(story) === vote ? 0 : vote, id = storyKey(story), list = current().feedback || (current().feedback = []);
    var mine = list.filter(function(item) { return storyKey(item) === id; })[0];
    if (mine) mine.vote = next;
    else list.push({headline:story.headline, source:story.source || '', section:story.section || '', url:story.url || '', vote:next});
    persist(); render();
    return next;
  };
  // Every stored day's votes, in the shape of the synced doc's entries map, so
  // the copied AI prompt summarises them exactly as the server generator does.
  root.dailyBoostFeedbackEntries = function() {
    var out = {};
    if (!account) return out;
    Object.keys(records).forEach(function(key) { if ((records[key].feedback || []).length) out[key] = {feedback:records[key].feedback}; });
    return out;
  };
  ...
  // A dated check from elsewhere in the app (Today's aha → Check this later). It
  // joins To check, so it comes back on its day and in the Morning 5, and it
  // never takes the day's one experiment slot. Returns {ok, message}.
  root.addDailyBoostCheck = function(check) {
    if (!check || !String(check.metric || '').trim() || !String(check.headline || '').trim() || !el('note')) return {ok:false, message:'Nothing to check.'};
    render();
    if (!account) return {ok:false, message:'Sign in to save a check.'};
    var due = String(check.due || ''), parsed = new Date(due + 'T00:00:00Z');
    if (!DAY_KEY.test(due) || !Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0,10) !== due || due < day) return {ok:false, message:'Choose a check date of today or later.'};
    // No closing full stop: the panel follows the metric with ", in 7 days".
    var metric = String(check.metric).trim().replace(/[.\s]+$/, '').slice(0, METRIC_CHARS), headline = String(check.headline).trim().slice(0,200);
    var existing = allReminders().filter(function(entry) { return !entry.item.done && entry.item.metric === metric && entry.item.headline === headline; })[0];
    if (existing) return {ok:false, message:'Already in To check for ' + label(existing.item.due) + '.'};
    var list = current().reminders || (current().reminders = []);
    if (list.filter(function(item) { return !item.done; }).length >= 10) return {ok:false, message:'Today already has ten open checks. Tick one off first.'};
    if (list.length >= 30) return {ok:false, message:'Today has reached its limit of thirty saved checks. Your findings are kept; add another tomorrow.'};
    list.push({metric:metric, headline:headline, source:String(check.source || ''), url:String(check.url || ''), due:due, done:false, doneOn:''});
    commitDays([day]);
    return {ok:true, message:'Added to To check for ' + label(due) + '. It comes back that day, and in your Morning 5.'};
  };
  ...
  // Finding editors left open, by day and check, so a rebuild keeps them open.
  var openFindings = {};
  function renderReminders() {
    var panel = el('reminders');
    if (!panel) return;
    // Every confirmed save re-renders Today. Rebuilding the list while a finding
    // is being typed closed the box and dropped the caret about two seconds after
    // each pause; the list catches up on the next render instead.
    var focused = document.activeElement;
    if (focused && String(focused.tagName || '').toUpperCase() === 'TEXTAREA' && panel.contains && panel.contains(focused)) return;
  ...
  function reminderRow(entry, isDue) {
    var rowKey = entry.day + '|' + itemKey(entry.item);
    ...
    // A different due day, applied as soon as it is picked.
    var dateLabel = document.createElement('label'), dateInput = document.createElement('input');
    dateLabel.className = 'watch-reminder-date'; dateLabel.textContent = 'Due ';
    dateInput.type = 'date'; dateInput.value = item.due; dateInput.setAttribute('aria-label', 'Change the due date for ' + item.metric);
    dateLabel.appendChild(dateInput);
    dateInput.addEventListener('change',function() {
      var value = dateInput.value, parsed = new Date(value + 'T00:00:00Z');
      if (!DAY_KEY.test(value) || !Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0,10) !== value || value === item.due) return;
      item.due = value; commitDays([entry.day]);
    });
    [['✓ Checked', function() { reminderAction(entry, 'done'); }],
     ['✎ Note what you found', function() { openFindings[rowKey] = true; editor.hidden = false; finding.focus(); }],
    ...
    var editor = document.createElement('div'), finding = document.createElement('textarea'), findingLabel = document.createElement('label'), save = document.createElement('button'), message = document.createElement('p');
    editor.hidden = !openFindings[rowKey]; editor.className = 'boost-reflect'; findingLabel.textContent = 'What did you find?'; finding.maxLength = 400; finding.rows = 2; finding.value = item.finding || '';
    findingLabel.appendChild(finding); save.className = 'tool-chip'; save.textContent = 'Save finding & check off'; message.setAttribute('role','status');
    var owner = account, reminderId = itemKey(item);
    function currentCheck() {
      return account === owner && root._firebaseUid === owner && records[entry.day] && (records[entry.day].reminders || []).find(function(check) { return itemKey(check) === reminderId; });
    }
    finding.addEventListener('input',function() {
      var check = currentCheck(); if (!check) return;
      check.finding = finding.value.slice(0,400); records[entry.day].updatedAt = Date.now(); sync.dirty[entry.day] = true; saveLocal(); schedule(); status();
    });
    save.addEventListener('click',function() {
      if (!finding.value.trim()) { message.textContent = 'Write a finding first, or use Checked if no note is needed.'; return; }
      var check = currentCheck();
      if (!check) { message.textContent = 'This check changed. Reopen it to save your finding.'; return; }
      check.finding = finding.value.trim().slice(0,400); check.done = true; check.doneOn = day; delete openFindings[rowKey]; commitDays([entry.day]);
    });
    editor.append(findingLabel,save,message); row.appendChild(editor);
    return row;
  }
  ...
  root.createDailyBoostExperiment = function(action, due, source) {
    render();
    if (!account) return {ok:false,message:'Sign in to save an experiment.'};
    var parsed = new Date(due + 'T00:00:00Z');
    if (!String(action || '').trim() || !DAY_KEY.test(due || '') || !Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0,10) !== due || due < day) return {ok:false,message:'Choose a review date of today or later.'};
    if (current().trialPlan) return {ok:false,message:'You already have an experiment today. It is kept in Your experiments below; this suggestion has not replaced it.'};
    current().trialPlan = String(action).trim().slice(0,400); current().trialDue = due; current().trialSource = String(source || '').slice(0,SOURCE_CHARS); current().trialDone = false;
    persist(); render();
    return {ok:true,message:'Saved to Your experiments. Revisit on ' + label(due) + '.'};
  };
  root.noteDailyBoostInsight = function(title) {
    render(); if (!account) return false;
    var lead = 'My take on “' + String(title).slice(0,140) + '”: ';
    if (current().note.indexOf(lead) < 0 && current().note.length + lead.length + 2 > 1200) {
      focusNote(); el('feedback').textContent = 'Your reflection is full. Make room before adding your take; your existing note is unchanged.'; return false;
    }
    prefillNote(lead,title); return true;
  };
  root.resetDailyBoost = function() { openFindings = {}; records = {}; archive = {}; archiveFor = null; account = null; day = null; expanded = false; shownDay = null; resetSync(); resetNoteEdit(); if (el('note')) { el('note').value = ''; el('history').replaceChildren(); el('feedback').textContent = ''; el('export').value = ''; el('export').hidden = true; el('copy-status').textContent = ''; el('noted').replaceChildren(); el('noted').hidden = true; } };
  document.addEventListener('DOMContentLoaded',function() {
    ...
    el('swap').addEventListener('click',function() {
      render();
      if (!current().done) { current().spark = current().intention ? chooseForIntention(current().intention) : nextSpark(current().spark,records,day,goalLean()); current().picked = 'swap'; current().ref = ''; current().refSource = ''; current().refUrl = ''; }
      persist(); render();
    });
    ...
    ['helpful','not-today','more'].forEach(function(value) {
      el('rate-' + value).addEventListener('click',function() {
        render(); if (!account) return;
        var previous = current().sparkResponse;
        current().sparkResponse = previous && previous.spark === current().spark && previous.value === value ? null : {spark:current().spark,value:value,note:previous && previous.spark === current().spark ? previous.note : ''};
        persist(); render();
      });
    });
    el('rate-note').addEventListener('input',function() {
      if (account !== root._firebaseUid || !current().sparkResponse || current().sparkResponse.spark !== current().spark) return;
      current().sparkResponse.note = el('rate-note').value.slice(0,240); persist();
    });
    ...
    el('trial-save').addEventListener('click',function() {
      ...
      persist(); render(); el('trial-status').textContent = 'Saved. It comes back in Your experiments on ' + label(current().trialDue) + '.';
    });
    el('done').addEventListener('click',toggleDone);
    if (el('compact-week')) el('compact-week').addEventListener('click',openWeeklyRead);
    if (el('goals-edit')) el('goals-edit').addEventListener('click',function() { goalsEditing = true; goalsMessage = ''; render(); });
    if (el('start')) { el('start-go').addEventListener('click',startGo); el('start-skip').addEventListener('click',startSkip); }
    if (el('past')) [['use','used'],['later','later'],['dismiss','dismissed']].forEach(function(pair) { el('past-' + pair[0]).addEventListener('click',function() { discoveryAct(pair[1]); }); });
    ...
    el('note').addEventListener('input',function() {
      ...
      current().note = el('note').value.slice(0,1200); noteTyped(); persist(); renderStart();
    });
    // Done appears while typing; leaving the box hides it (after a beat, so a tap
    // on Done itself still lands). Ctrl/Cmd+Enter is Done from the keyboard.
    el('note').addEventListener('blur',function() {
      root.setTimeout(function() { if (document.activeElement !== el('note')) el('note-done').hidden = true; },200);
    });
    el('note').addEventListener('keydown',function(event) {
      if ((event.ctrlKey || event.metaKey) && event.key === 'Enter') { event.preventDefault(); noteDone(); }
    });
    el('note-done').addEventListener('mousedown',function(event) { event.preventDefault(); });
    el('note-done').addEventListener('click',noteDone);
