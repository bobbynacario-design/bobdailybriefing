...
(function(root) {
  ...
  function promptFor(day, offset) {
    return prompts[indexFor(day,offset,prompts.length)];
  }
  ...
  root.DailyBoostCore = {dateKey:dateKey,promptFor:promptFor,clean:clean,count:prompts.length};
  if (!root.document) return;
  var records = {}, account = null, day = null, failed = false;
  function el(id) { return document.getElementById('boost-' + id); }
  function current() { return records[day] || (records[day] = {energy:'gentle',spark:indexFor(day,0,prompts.length),offset:0,done:false,note:''}); }
  ...
  document.addEventListener('DOMContentLoaded',function() {
    ...
    el('swap').addEventListener('click',function() { render(); if (!current().done) current().spark = (current().spark + 1) % prompts.length; persist(); render(); });
