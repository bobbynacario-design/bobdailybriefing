...
(function(root) {
  ...
  function clean(value) {
    ...
    days.forEach(function(day) {
      ...
      result[day] = {energy:item.energy === 'stretch' ? 'stretch' : 'gentle', offset:Number.isInteger(item.offset) && item.offset >= 0 ? item.offset % prompts.length : 0, done:item.done === true, tucked:item.tucked === true, note:typeof item.note === 'string' ? item.note.slice(0,1200) : '', carry:typeof item.carry === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(item.carry) ? item.carry : '', ref:typeof item.ref === 'string' ? item.ref.slice(0,300) : '', refSource:typeof item.refSource === 'string' ? item.refSource.slice(0,120) : '', refUrl:typeof item.refUrl === 'string' && /^https?:\/\//i.test(item.refUrl) ? item.refUrl.slice(0,600) : '', picked:item.picked === 'library' || item.picked === 'swap' ? item.picked : '', stories:cleanStories(item.stories), reminders:cleanReminders(item.reminders), opened:day >= openedSince ? cleanOpened(item.opened) : [], updatedAt:Number.isFinite(item.updatedAt) && item.updatedAt > 0 ? Math.floor(item.updatedAt) : 0};
  ...
  function whyText(item) {
    var parts = [item.picked === 'library' ? 'You picked this from the library' : item.picked === 'swap' ? 'You swapped to this one' : item.spark === REVIEW && isSunday(day) ? 'Sunday look-back' : 'From today’s rotation'];
  ...
  function render() {
    ...
    if (switched) {
      account = uid; records = {}; failed = false; shownDay = null; resetSync();
  ...
  function exportText() {
    var days = Object.keys(records).filter(function(key) { return won(records[key]) || (records[key].reminders || []).length; }).sort().reverse();
  ...
  document.addEventListener('DOMContentLoaded',function() {
    ...
    el('swap').addEventListener('click',function() {
      render();
      if (!current().done) { current().spark = nextSpark(current().spark,records,day); current().picked = 'swap'; current().ref = ''; current().refSource = ''; current().refUrl = ''; }
      persist(); render();
    });
