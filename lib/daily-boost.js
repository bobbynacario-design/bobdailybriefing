...
(function(root) {
  ...
  // order of what will not wait: a check that is due, an experiment ready to
  // review, the Sunday read, today's spark, then a line in the note. "Not now"
  // moves on for the rest of this visit; nothing else on the page is hidden.
  var START_NAMES = {check:'a check', review:'an experiment review', read:'your weekly read', spark:'today’s spark', note:'a line in your note'};
  ...
  function renderGoals() {
    ...
    if (!goalsEditing) {
      ...
      list.forEach(function(goal) {
        ...
        row.append(title, meta, dots); box.appendChild(row);
      });
      return;
    }
    var rows = list.slice();
    ...
    var inputs = rows.map(function(goal, index) {
      var row = document.createElement('div'), text = document.createElement('input'), theme = document.createElement('select');
      ...
      row.append(text, theme); editor.appendChild(row);
      return {goal:goal, text:text, theme:theme};
    });
  ...
  root.dailyBoostGoals = function() { return activeGoals().map(function(goal) { return {id:goal.id, text:goal.text, theme:goal.theme}; }); };
