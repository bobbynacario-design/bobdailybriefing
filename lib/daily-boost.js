...
(function(root) {
  ...
  var FIELDS = ['energy','spark','done','tucked','note','carry','ref','refSource','refUrl','picked','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource'];
  ...
  var ARCHIVE_FIELDS = ['energy','spark','done','note','carry','ref','refSource','refUrl','stories','reminders','intention','trialPlan','trialDue','trialOutcome','trialDone','trialSource','updatedAt'];
  ...
  function worthKeeping(entry) {
    return !!entry && (won(entry) || entry.stories.length || entry.reminders.length || !!entry.trialPlan || !!entry.ref);
  }
  ...
  function exportText() {
    var days = allDays().filter(function(key) { var entry = dayEntry(key); return won(entry) || entry.trialPlan || (entry.reminders || []).length; }).reverse();
  ...
  root.addDailyBoostCheck = function(check) {
    ...
    if (list.length >= 10) return {ok:false, message:'Today already has ten checks. Tick one off first.'};
  ...
  root.resetDailyBoost = function() { records = {}; archive = {}; archiveFor = null; account = null; day = null; expanded = false; shownDay = null; resetSync(); resetNoteEdit(); if (el('note')) { el('note').value = ''; el('history').replaceChildren(); el('feedback').textContent = ''; el('export').value = ''; el('export').hidden = true; el('copy-status').textContent = ''; el('noted').replaceChildren(); el('noted').hidden = true; } };
