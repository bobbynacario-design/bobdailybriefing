...
function setup({now = '2026-09-26T04:00:00Z', uid = 'alice', ...extra} = {}) {
  ...
  const make = (tag) => ({tag, value:'', textContent:'', className:'', children:[], handlers:{}, hidden:false, disabled:false, open:false,
    addEventListener(name, fn) { this.handlers[name] = fn; }, setAttribute(name, value) { this[name] = String(value); }, replaceChildren() { this.children = []; },
...
test('theme days open exact entries; Try this and Open as decision keep provenance',async()=>{
  let opened, created, decision;
  const {context,element}=setup({fbLoadWeeklyMirror:async()=>({'2026-09-26':MIRROR}),openDailyBoostDay:key=>{opened=key;return true;},createDailyBoostExperiment:(...args)=>{created=args;return {ok:true,message:'Saved experiment'};},openDecisionFromInsight:value=>{decision=value;}});
  ...
  nodes.find(n=>n.tag==='button' && n.textContent==='Open as decision').handlers.click();
  assert.equal(decision.title,MIRROR.try_next.action); assert.equal(decision.source,'weekly'); assert.equal(decision.reviewDate,'2026-09-30'); assert.match(decision.detail,/2026-09-26/);
...
test('while it reads, the status carries the app\'s live ring and timer, then clears', async () => {
  let release; const seen = [];
  const {context, element} = setup({fbLoadWeeklyMirror:async () => ({}), fbGenerateWeeklyMirror:() => new Promise((resolve) => { release = resolve; }),
    aiWorkingHtml:(text, opts) => { seen.push({text, opts}); return '<working>' + text + '</working>'; }});
  context.renderWeeklyMirror(); await tick();
  element('run').handlers.click();
  assert.equal(element('status').innerHTML, '<working>Reading your week. Usually about half a minute; your notes go to OpenAI for this read only.</working>');
  const last = seen[seen.length - 1];
  assert.equal(last.opts.since, Date.parse('2026-09-26T04:00:00Z'), 'timed from the tap');
  assert.equal(last.opts.slow, 90);
  release({mirror:MIRROR}); await tick();
  assert.equal(element('status').textContent, '', 'plain text again once it is done');
});
...
// ── Write my answer ──
test('the question can be answered, saved with its question, edited and cleared',async()=>{
  const saves=[];
  const {context,element}=setup({fbLoadWeeklyMirror:async()=>({'2026-09-26':JSON.parse(JSON.stringify(MIRROR))}),fbSaveWeeklyMirrorAnswer:async(uid,key,answer)=>{saves.push({uid,key,answer});}});
  context.renderWeeklyMirror(); await tick();
  const nodes=()=>descendants(element('body')), button=text=>nodes().find(n=>n.tag==='button'&&n.textContent===text);
  button('Write my answer').handlers.click();
  const box=nodes().find(n=>n.tag==='textarea');
  box.value='  A ten-minute note before email.  '; box.handlers.input();
  button('Save answer').handlers.click(); await tick();
  assert.equal(saves.length,1);
  assert.equal(saves[0].uid,'alice'); assert.equal(saves[0].key,'2026-09-26');
  assert.equal(saves[0].answer.text,'A ten-minute note before email.');
  assert.equal(saves[0].answer.question,MIRROR.question,'kept with the question it answers');
  assert.ok(nodes().some(n=>n.textContent==='A ten-minute note before email.'),'shown under the question');
  assert.ok(nodes().some(n=>n.textContent==='Saved. Your next read will follow up on it.'));
  button('Edit my answer').handlers.click();
  const again=nodes().find(n=>n.tag==='textarea');
  assert.equal(again.value,'A ten-minute note before email.','edit starts from the saved answer');
  again.value=''; again.handlers.input(); button('Save answer').handlers.click(); await tick();
  assert.equal(saves[1].answer,null,'an empty answer clears it');
  assert.ok(button('Write my answer'));
});
test('a failed save keeps the answer in the box, and an answer to an earlier wording says so',async()=>{
  const reworded=Object.assign({},MIRROR,{answer:{text:'Mornings.',question:'An earlier wording?'}});
  const {context,element}=setup({fbLoadWeeklyMirror:async()=>({'2026-09-26':reworded}),fbSaveWeeklyMirrorAnswer:async()=>{throw Error('denied');}});
  context.renderWeeklyMirror(); await tick();
  const nodes=()=>descendants(element('body'));
  assert.ok(nodes().some(n=>n.textContent==='To an earlier wording: “An earlier wording?”'));
  nodes().find(n=>n.tag==='button'&&n.textContent==='Edit my answer').handlers.click();
  const box=nodes().find(n=>n.tag==='textarea'); box.value='Mornings, before email.'; box.handlers.input();
  nodes().find(n=>n.tag==='button'&&n.textContent==='Save answer').handlers.click(); await tick();
  assert.equal(nodes().find(n=>n.tag==='textarea').value,'Mornings, before email.');
  assert.ok(nodes().some(n=>/Could not save your answer\. It is still in the box/.test(n.textContent)));
});
test('the read answers each of your goals',async()=>{
  const withGoals=Object.assign({},MIRROR,{goals:[{goal:'Be consistent with what I want',read:'You ticked it on 2026-09-22 and Friday.'},{goal:'Build a BI advisory offer',read:''}]});
  const {context,element}=setup({fbLoadWeeklyMirror:async()=>({'2026-09-26':withGoals})});
  context.renderWeeklyMirror(); await tick();
  const all=textOf(element('body'));
  assert.ok(all.includes('Your goals this week'));
  assert.ok(all.includes('Be consistent with what I want — You ticked it on 2026-09-22 and Friday.'));
  assert.ok(all.includes('Build a BI advisory offer'));
  const {context:c2,element:e2}=setup({fbLoadWeeklyMirror:async()=>({'2026-09-26':MIRROR})});
  c2.renderWeeklyMirror(); await tick();
  assert.ok(!textOf(e2('body')).includes('Your goals this week'),'no goals, no part');
});
