...
// The second matters because every OpenAI-COMPATIBLE provider (Ollama, DeepSeek,
// Qwen, GLM, OpenRouter) speaks chat/completions. Reading only the first set
// would record zero tokens for those without failing, which is worse than an
// error: the cost ledger would look clean and be wrong.
function extractUsage(json) {
  var u = (json && json.usage) || {};
  var inTok = num(u.input_tokens != null ? u.input_tokens
    : (u.prompt_tokens != null ? u.prompt_tokens : u.inputTokens));
  var outTok = num(u.output_tokens != null ? u.output_tokens
    : (u.completion_tokens != null ? u.completion_tokens : u.outputTokens));
  var cached = 0;
  if (u.input_tokens_details && u.input_tokens_details.cached_tokens != null) {
    cached = num(u.input_tokens_details.cached_tokens);
  } else if (u.prompt_tokens_details && u.prompt_tokens_details.cached_tokens != null) {
    cached = num(u.prompt_tokens_details.cached_tokens);   // chat/completions
