...
test('every prompt asks for one aha, with the rules that keep it honest', () => {
  ...
  assert.match(prompt, /Among reads that hold, prefer the one that would be early rather than safe; never trade coherence for surprise\./);
  // The 26 Sep regenerate joined two stories on the word "storm".
  assert.match(prompt, /A connection needs a mechanism: the same actor, market, cost, place or cause running from one story into the other\. Sharing a word or a theme/);
  assert.match(prompt, /Every linked story must be necessary: if the read still stands with one of them removed, it is not a connection\./);
  assert.match(prompt, /An event that causes no loss \(a storm forecast to miss land\) cannot carry a claims insight\./);
  assert.match(prompt, /If no two stories genuinely act on each other today, set aha to null/);
...
test('the prompt says what his work turns on, and asks the relevance line to name the number it moves', () => {
  const prompt = buildBriefingPrompt({dateLabel: DATE});
  assert.match(prompt, /claims-quantum specialist working for Australian insurers \(mainly QBE and Allianz\) and their solicitors/);
  assert.match(prompt, /HIS WORK — what the insurance, interruptions and global sections are for:/);
  assert.match(prompt, /vehicles striking utility and road assets/);
  assert.match(prompt, /loss of income for heavy-vehicle operators/);
  assert.match(prompt, /first-party business interruption for small businesses/);
  assert.match(prompt, /AER determinations, linesworker and contractor rates/);
  assert.match(prompt, /If the best you can say is that insurers are exposed, the story has not earned its place\./);
  assert.match(prompt, /This steers priority only\. It never overrides grounding/);
});

test('his accounts: a starter list, cleaned, matched as whole words, and given to the briefing as private context', () => {
  const {DEFAULT_ACCOUNTS, cleanAccounts, accountsInText} = context.BriefingPromptCore;
  assert.ok(DEFAULT_ACCOUNTS.some((a) => a.name === 'QBE') && DEFAULT_ACCOUNTS.some((a) => a.name === 'SA Power Networks'));
  const clean = cleanAccounts([{name: ' QBE ', aliases: ['QBE Insurance', ''], kind: 'insurer'}, {name: 'qbe'}, {name: ''}, {name: 'New firm', kind: 'weird'}, null]);
  assert.deepEqual(clean.map((a) => a.name), ['QBE', 'New firm'], 'duplicates and blanks dropped');
  assert.equal(clean[1].kind, 'other');
  assert.equal(cleanAccounts(Array.from({length: 60}, (_, i) => ({name: 'A' + i}))).length, 40);
  const list = [{name: 'Ergon Energy', aliases: ['Ergon']}, {name: 'SA Power Networks', aliases: ['SAPN']}, {name: 'Hall & Wilcox'}, {name: 'QBE'}];
  assert.deepEqual([...accountsInText('SAPN lifts pole charges; Hall & Wilcox acts', list)], ['SA Power Networks', 'Hall & Wilcox']);
  assert.deepEqual([...accountsInText('An ergonomic chair for QBE staff', list)], ['QBE'], 'whole words only: ergonomic is not Ergon');
  assert.deepEqual([...accountsInText('Ergon crews restore power', list)], ['Ergon Energy']);
  assert.deepEqual([...accountsInText('', list)], []);
  const prompt = buildBriefingPrompt({dateLabel: DATE, context: Object.assign({}, context_, {accounts: list})});
  assert.match(prompt, /HIS ACCOUNTS:\n- A HIS ACCOUNTS block near the end of this prompt names the organisations his files involve/);
  assert.match(prompt, /Search for news on them\./);
  assert.match(prompt, /never report the list/);
  assert.match(prompt, /HIS ACCOUNTS — private context, not news:\nErgon Energy \(Ergon\); SA Power Networks \(SAPN\); Hall & Wilcox; QBE/);
  assert.doesNotMatch(buildBriefingPrompt({dateLabel: DATE}), /HIS ACCOUNTS/, 'no list, no block');
});

// The 26 Sep regenerate: an account name-dropped, generic watch metrics, an
// index-close story, and nothing from his beats.
test('accounts are named only when the story is about them, and his beats are searched first', () => {
  const prompt = buildBriefingPrompt({dateLabel: DATE, context: Object.assign({}, context_, {accounts: [{name: 'QBE'}]})});
  assert.match(prompt, /Name an account in a relevance line only when the story itself is about that organisation\. Never add an account's name to a story that does not mention it/);
  assert.match(prompt, /Before filling the interruptions and global sections with general news, search specifically for: electricity network charges/);
  assert.match(prompt, /heavy-vehicle repair times, parts shortages, prime-mover hire and freight rates/);
});
test('a watch metric must resolve on a known day, and markets never run an index move', () => {
  const prompt = buildBriefingPrompt({dateLabel: DATE});
  assert.match(prompt, /It must be something that will resolve or change on a known day: a named decision, hearing, release or data point\. A database or page that simply updates every day or week/);
  assert.match(prompt, /In the markets section, never run a story whose headline is an index or currency move/);
});
test('the fetched list carries network and trucking news, used in insurance only when it moves a claim', () => {
  const prompt = buildBriefingPrompt({dateLabel: DATE, evidence});
  assert.match(prompt, /insurance trade news, plus electricity-network and trucking trade news for the costs his claims turn on/);
  assert.match(prompt, /A network or trucking story belongs in insurance only when it moves a cost or a claim in his files/);
});
